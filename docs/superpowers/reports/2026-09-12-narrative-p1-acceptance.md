...
早期四项工作的证据状态如下；最新核心通过以 Task 16 为准。正式六矩阵和创建到终局的实机 UI 验收没有执行，不记为通过，也不进入 P2。历史 `p1-07` 的 0/6 来自六次独立开局，不是新协议矩阵。根因与后续架构方向见 [失败分析](2026-09-13-narrative-p1-failure-analysis.md)。
...
旧主线将泛化纠纷交给 talk/取物/战斗任务推进，中心问题不一定能由现有规则解决。本项将首个验收输入收敛为公开递送：明确委托 → 听取路线与不同意见 → 给绑定接应人实际交付。AI 仍创作人物、对白、阻碍和结尾，不使用固定开局或替代剧情。

生产回归确定复现了最终换幕 `approval:bundle_missing_step`：作者/编译投影只有 move，物化后的审批图要求 move → give_item。修复只对齐已有投影、槽位选择与展示时序，底层交付和终局规则未改。作者/审阅共用当前归属、绑定接收人、成功交付事件与可声明公开事实；普通公开约定可以作为交付依据，递送换幕不强制生成无关物品或敌人，NPC 仍显式声明 existingFactIds。

独立实现审核 Spec/Quality 通过。131 项定向测试、typecheck、131 项 boundaries 和 docs 检查通过；完整测试以 2 workers 运行，2842 passed、1 skipped。默认并行的两轮仅文件扫描检查超时，未改断言或阈值，日志保留。lint 0 errors、50 项既有 warning。

### 唯一冻结故事

- 代码：`f95883a8`；协议：`4f3905444836c6ff86d824a048bba467c62577b2d06e698eb096e30e1f29b83f`，claimScope=`production_core_story`，路线 `S1-complete`，24 行动/200 HTTP/90 分钟上限。实际 10 行动、27 HTTP，无耗尽后的手动 retry、无换样本。
- 正式创建 game `4fc46756-3ad3-4c27-b7d7-9c4c56bca661`。王氏委托沈行把信交给周良；经河堤渡口听取陈水根的意见，抵达柳溪村并实际递交原信件。
- 第 7 回合 `item_0` 交给 `npc_dyn_2`，唯一成功 `item_given` 与当前 NPC owner 一致；第 10 回合产生唯一 `ending_dyn_0` 成功事件，revision 16、narrative ready，三项主线完成。
- 四行动后正式关闭重开，状态一致。严格 replay 为 HTTP 0、27 响应/15 状态匹配；live/replay SQLite integrity 均为 ok，验证结果一致。
- 终幕批准回合 9、实际展示回合 10；场景、History 与结局事件均在 10，正文精确来自对应获批 endingOutcomes。

### 正文结论与边界

核心正文通过：委托动机明确，路上不同意见与涨水构成有界阻碍，玩家通过真实对话、移动、交付和最后立场完成故事，收信人的当场回应承接已递交的公开来信。生成中的越界说法曾被审阅拒绝并在原候选预算内修订，没有绕过知识审批。

结尾“旧账/心结”只按人物对来信的情感回应理解；没有实际付款事件，也没有据此改变王氏与周良的持久化关系。不能把该措辞当成债务支付、双边关系更新或额外世界行动已执行的证据。不为文学动作逐一补状态，不开启措辞润色批次。

本项证明单条生产核心闭环可行，未证明多路线分化、自由输入全覆盖、整体质量优于 main 或整个 P1 已验收。旧失败样本及其分母保持，下一步入口仍是 Plan 中已有的交付/退出专项；本轮未扩展执行。

证据：`artifacts/narrative-p1/delivery-core-01/` 的 protocol.json、live/replay/summary.json、verification.json、complete-story.md、acceptance-review.md 与原始磁带/审计/SQLite；过程审核与门禁日志在 `.superpowers/sdd/2026-09-12-narrative-architecture-p1/`。
