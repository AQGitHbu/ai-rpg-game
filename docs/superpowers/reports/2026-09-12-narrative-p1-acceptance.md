...
P1 仍未通过。历史完整批次 `p1-07` 为 0/6、HTTP 44 次；旧 runner 实际为六次独立开局，不能称为同开局三策略的固定矩阵。离线测试证明部分规则和持久化能力，但 P1-A 的保密—真实引荐闭环仍有缺口，P1-B 原生产调用链也有断点。根因、修复与复核范围见 [失败分析](2026-09-13-narrative-p1-failure-analysis.md)。
...
后续先补齐具体保密条件与真实引荐的规则闭环，再按协议 v2 新建批次。修复后的离线门禁见 [失败分析](2026-09-13-narrative-p1-failure-analysis.md)；本报告上述门禁数字为原批次证据，不代表此次修复已经完成 live 验收。响应 replay 尚未实现，CLI 明确拒绝，不能把摘要标记当作重放通过。
