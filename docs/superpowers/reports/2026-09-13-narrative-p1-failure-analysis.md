...
- runner 改为两次获批初始化、关闭并 checkpoint 后各复制三份 SQLite；按已批准操作和真实结算事件推进，核验自由输入绑定实际交付机会和接应 NPC。HTTP 使用差分统计，预算/中断写出部分 summary。旧 replay 仅相信摘要的实现被删除，现明确拒绝；正式响应 replay 仍待实现。
...
现有私下 fixture 用 `promise_confidentiality` 配“私下请求引荐”文案，没有实际执行独立 `request_introduction`；任务完成的通用 `kept_promise` 信号会关闭匹配承诺，未验证特定保密条件。七条 P1-A 测试通过不能证明这一完整语义闭环。因此 P1-A 的相关验收重新打开，后续应先在 Task 3/4 定义并验证具体承诺履行条件、真实引荐和违约分支，再启动新 live 矩阵，而非继续调模型参数。
