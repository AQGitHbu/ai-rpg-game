...
最新剩余问题集中在开局创作与编译边界。diag04 的前两份原始响应分别将 `$.opening.item` 和 `$.opening.location/npc/quest/situation/item` 放错位置，正确容器为 `$.opening.opening`。原样送入生产 source 可复现 `opening_unknown_keys`；下一版只得到该通用码，没有违规字段路径。第三版结构正确，但正文明确暗示追查者可能藏于客栈，与 NPC 的私密 fact_7 相符，审阅要求修订时三个候选版本已经用尽。

这说明三个版本的端到端修订预算同时承担结构组装、权限校验与叙事质量修订；结构错误可以耗尽语义修订机会。它是可复现的可靠性瓶颈，而非 token 数或长记忆容量不足的证据。审阅阻止秘密提前发布是正确行为，不应通过放宽评分或忽略拒绝来取得终局。

下一次实施应先在总 Spec/P1 Plan 中明确生成契约的收敛方案：由程序负责固定结构与权威引用，AI 负责创作内容与具体选择意图；结构错误提供精确路径及待修订候选；私密世界材料与玩家可见文本在权限明确后分别编译。是否调整结构修复与语义候选预算，也须明确版本身份、崩溃恢复和计数，不能暗加无限重试。这些是建议方向，尚未实现；不应直接回到逐条追加提示或连续重抽 live 样本。
