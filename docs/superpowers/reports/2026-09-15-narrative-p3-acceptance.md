...
P3 已有核心调查、合作与离线双路线证据；[本轮收尾](2026-09-16-narrative-p3-closeout.md)接通 B 本场披露的知识事件及同候选后果预览，完成匠人题材的最小规则复用。完整故事中的实际回访、实际告知及合法返程也已通过 SQLite 与 raw 编译链验证。真实批次 `p3-closeout-01` 暴露生成/审阅状态混用，修复后的 `p3-closeout-02` 已成功换幕，但暴露作者绑定 schema 缺失。补齐契约后，`p3-closeout-03` 候选已生成双调查方法，但因结构错误及后续传播来源违规耗尽有限尝试。三批合计 34 次 HTTP，均未到 UI 分叉，严格零 HTTP 回放保持 blocked。下文保留 `p3-01` 原结果；具体新批次证据集中在收尾报告。没有完整 live/UI 证据，P3 整体仍不通过。
...
| C7 完成性 | Task 7 离线双路线；Task 8 runner 保留真实批次两路状态及分母 | 真实批次 blocked，UI 未进入，不通过 |
