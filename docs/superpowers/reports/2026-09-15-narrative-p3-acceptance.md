...
P3 已有核心调查、合作与离线双路线证据；[本轮收尾](2026-09-16-narrative-p3-closeout.md)接通 B 本场披露的知识事件及同候选后果预览，完成匠人题材的最小规则复用。完整故事中的实际回访仍待完成，调查后重载不构成回访；UI 验收入口已准备，但本轮尚未执行真实批次。下文保留 `p3-01` 两路 blocked 和严格 replay 的原结果，不作为故事通过证据。没有完整 live/UI 证据，P3 整体仍不通过。
...
| C8 可维护性 | 复用 P1 replay/SQLite；拒绝有限调查依赖环；匠人材料查证、实际告知与新方法开放 | 模块与另一题材片段通过，完整故事的必经路径待验收 |
...
因此当前总判定仍为：P3 尚不能标记为整体完成；需补齐同故事实际回访，再以同一真实批次提供 C7 要求的 live/UI 证据。上述 Task 8 检查与下文 `p3-01` 保留原批次结果，本轮修复、检查和未完成边界见收尾报告。
