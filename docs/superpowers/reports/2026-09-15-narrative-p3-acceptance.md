...
P3 已有核心调查、合作与离线双路线证据；[代码审阅](2026-09-15-narrative-p3-code-review.md)发现并修复了规则和恢复缺陷，同时确认 B 本场披露的知识事件及同候选后果预览仍不完整，不能将 P3-B 整体记为完成。双路线 fixture 的调查后重载也不构成实际回访。真实批次 `p3-01` 的 private/public 都以 `ROUTE_POLICY_UNSUPPORTED` blocked，未形成两种调查方法、合作分化、合法交付/终局；严格 replay 通过但不是故事通过。没有实际 UI 证据，因此 P3 整体不通过。
...
因此当前总判定仍为：P3 尚不能标记为整体完成；需补齐上述 B 模块和离线场景缺口，再以同一真实批次提供 C7 要求的 live/UI 证据，确认没有程序性越权或缺少必要上下文。以下 Task 8 检查与 `p3-01` 保留原批次结果，本轮修复的检查另见代码审阅报告。
