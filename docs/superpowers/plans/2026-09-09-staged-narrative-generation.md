...
3. 周期总时限 600 秒，单次 provider 45 秒、最多 2 次传输尝试；按剩余时间缩短 timeout，不能开始超时后重试。**实现偏离（已回写）**：规划阶段单次 provider 放宽到 90 秒——规划要产出 6000 maxTokens 的完整骨架 JSON（比表达单元大一个量级），45 秒对慢 provider 偏紧；表达单元维持 45 秒。runJob 的每次请求（含重试）都按 `min(默认超时, job.deadline 剩余时间)` 收缩，请求不会越过周期 deadline。输入预算规划 8000、旁白 2500、角色 4000、选项 2500 estimated tokens；输出上限分别 6000、1000、1600、600；thinking 默认 off。这些为初始性能约束，不保证最大规模包在慢 provider 下必然成功。基础 DAG 按每调用至少 1000ms 调度保留量做可容纳性检查，真实延迟另测。
