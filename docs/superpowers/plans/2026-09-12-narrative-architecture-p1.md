...
- [ ] 从 medium-story-01 保留存档独立复制并执行一次正式 retry，继续真实选择直至结局；新增调用单独限额登记，严格零网络重放新段并阅读全文。原失败运行不改为无中断通过。
...
- P1 总结论：未通过；首个恢复后的完整故事已取得，数据库替换与验证已完成。NPC 创建与权限契约修复已完成；原故事重试与独立中篇均已执行并严格重放，但未通关。下一处规则阻断是多 Thread 闭合与结局需求/入口不一致，需先闭合该契约；交付/退出、UI 与 main 对照分别记证据。
