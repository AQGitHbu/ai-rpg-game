...
**阅读：** Spec §4、§8、§12；[开发规范](../../游戏开发规范.md)、[连续性与记忆](../../agent/剧情连续性与结构化记忆.md)、[实体状态](../../agent/实体与组件世界状态.md)。
...
**阅读：** Spec §5、§7、§9；[NPC 系统](../../agent/NPC人格知识与关系图.md)、[行动裁决](../../agent/行动裁决.md)、[任务推进](../../agent/探索与任务推进.md)。
...
**Interfaces:** `TalkAction.interactionId?: string` 引用服务端已批准互动；该字段进入 action 重建、语义摘要及 choice token，不能只保留 label。新增 `resolveStoryInteraction(input: { worldState: WorldState; action: Action; eventId: EventId }): { ok: true; worldState: WorldState } | { ok: false; code: "MISSING_INTERACTION" | "PRECONDITION_FAILED" | "INVALID_EFFECT" }`，普通 talk 无此字段仍走原规则。成功的世界变更和既有回合事件由同一规则管线提交，纯函数自身不写数据库。
...
**阅读：** Spec §4.3、§5、§9；任务推进、战斗与结局；[MVP 玩家规则](../../策划文档/AI生成RPG_MVP.md)。
...
- [ ] 明确主动退出：已交付走完成；未交付返回委托人并归还则解除承诺；带物离开则放弃任务并保留持有/违约事实。结束画面可显示原 trust/doubt 立场，但结局事实由累计状态决定。
...
```ts
type TempleRoute = "private" | "public" | "verify_first" | "exit";
...
```ts
for (const route of ["private", "public", "verify_first", "exit"] as const) {
...
- [ ] 私下路线创建保密承诺换引荐；公开路线不要求先许保密承诺，通过既有证据核验；verify_first 在自由输入后停在持有信筒状态，生成真实 request_verification 选项，选中并核验成功后仍需显式 give_item；exit 跑完归还或带物退出的实际结局。
...
**P1-A gate：** 四条离线路线都有完整终局，规则与原文历史能解释差异。若只能靠不断新增故事专用 if 分支才能前进，停止进入 P1-B，先缩减/修正规则契约。

## P1-B：让真实 AI 使用这些能力

### Task 5：双向证据检索与持续对话指代

**阅读：** Spec §6；连续性与记忆；[ST 参考](../../设想/SillyTavern_Architecture_Analysis_for_AI_RPG.md)中关键词、作用域、递归与编译部分。
...
type NpcDeliberationProposal = Readonly<{
  ...
  interactionProposals: readonly StoryInteraction[];
}>;
...
- [ ] interactionProposals 引用 Task 2 四种操作，经当前目标、事实来源与所有权检查后才进入未提交候选；不能自授引荐资格。动态世界只允许眼前互动所需的局部 fact/互动或既定后续角色具象化，现有世界校验器仍约束引用、数量和时点；未来步骤不提交当前知识。
...
**Interfaces:** 保留 `NarrativeBundleSource.generate(context)` 的统一 opening/decision 出口；在 context/候选 envelope 增加 `candidateVersion` 和 outward NPC 提议，bundle 契约升至 **2**。`currentScene` 仍是一个正文源；其 choices 引用 Task 2 批准动作身份。
...
type CandidateReviewResult =
  | { ok: true; candidateVersion: number }
  | { ok: false; candidateVersion: number; defects: readonly CandidateDefect[] }
  | { ok: false; candidateVersion: number; failure: "PROVIDER_FAILURE" | "UNCERTAIN" };
```

`reviewNarrativeCandidate(input: { context: NarrativeBundleSourceContext; proposal: NarrativeBundleProposal | OpeningNarrativeBundleProposal; candidateVersion: number }): Promise<CandidateReviewResult>` 是 `NarrativeCandidateReviewer` 接口的方法，类型在 application 文件导出；live factory 依赖既有 `RpgAiClient`。review 只给缺陷，不写 mutation，不授知识。

审阅故障的 `failure` 分支直接进入显式失败；有缺陷的分支只有非空 defects 才能继续修订。通过必须显式 `ok: true` 并匹配版本，不能把空数组、解析失败或缺少结论推定通过。
...
```ts
if (review.candidateVersion !== candidateVersion) {
...
- [ ] opening 同样通过完整候选/审阅，但不为不存在的已物化 NPC 调用角色判断；初始化角色目标、证据与场景须共同一致。continuation 保留未选前不消费的语义；P1 live 作者默认只产当前完整场景，保留旧 continuation 的安全回归覆盖，不新增单位 DAG。
...
`ApplyStateInput.expectedNarrativeJob` 扩展 `epoch`、`leaseId` 作为 CAS 谓词；元数据更新可保持 revision，但必须匹配 job/status/epoch/lease。B 同时匹配 gameplay revision 与这些身份。时钟/UUID 在 application/server 注入，domain 不读取。
...
- [ ] 注册固定输入原文：**“短篇三幕，故事题材为破庙来信。玩家在破庙得到老信使帮助，接受将唯一信筒交给渡口接应人的委托；公开调查可能暴露信使行踪。须支持私下引荐、公开核验、先验身份再交付，以及主动退出。关键身份核验证据在开局建立来源，其他地点和角色随推进具象化。最多三个地点、三个 NPC。”** 游戏设置使用当前已支持的短篇值；其余必填创建字段在 register 命令中输出最终规范化 JSON 并冻结，live 不再修改。
- [ ] 冻结实际 `AI_MODEL` 非空值、API base（不含 key）、所有 purpose 的 thinking=`enabled`、JSON 模式=`prompt_only`，作者/NPC/review 均不设置 `max_tokens`，temperature 不额外覆盖。记录 provider 是否支持这些参数；不支持则 register 报配置错误，不静默切换。模型自称名称不能代替配置快照。
...
| --- | --- | --- |
| G1/G5、A1/A6/A10 | 2、3、4、9、10 | 具体动作契约、四条离线终局、六条 live 完整路线 |
