...
- [ ] **Step 7：提交与安全合入。** 记录实际测试结果、未满足项；确认 worktree clean 后提交本 Task 文件。后续实施统一使用 `codex/opening-context` 分支与 `.worktrees/opening-context` 工作区；用户授权合入时，从主仓 main 运行 `npm run branch:merge -- codex/opening-context`。未授权实施时整个 Task 保持未勾选。
