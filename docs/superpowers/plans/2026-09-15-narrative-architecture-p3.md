...
- **P3-C：** Task 7 离线双路线已证明调查、同故事实际回访、告知、合作分化及交付；另一题材片段已通过正式规则与独立复核。Task 8 已执行 `p3-closeout-01` 至 `04` 并完成严格零 HTTP 回放，两路仍 blocked、未进入 UI；生成/审阅状态混用、作者绑定 schema 缺失与未知字段反馈不精确已修复。保留真实验收缺口，停止围绕叙事内容继续抽样。具体失败、修复和批次证据见[收尾报告](../reports/2026-09-16-narrative-p3-closeout.md)。历史失败判定保留。
