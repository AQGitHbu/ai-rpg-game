...
- **P3-C：** Task 7 离线双路线已证明调查、同故事实际回访、告知、合作分化及交付；另一题材片段已通过正式规则与独立复核。Task 8 已重跑 `p3-closeout-01/02` 并完成严格零 HTTP 回放，两路仍 blocked、未进入 UI；首批生成/审阅状态混用已修复，第二批换幕成功但暴露作者绑定 schema 缺失。具体失败、修复和批次证据见[收尾报告](../reports/2026-09-16-narrative-p3-closeout.md)。历史失败判定保留。
