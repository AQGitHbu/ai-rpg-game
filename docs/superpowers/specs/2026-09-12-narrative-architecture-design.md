...
首个验收故事从正式新游戏创建开始，采用普通武侠渡口纠纷，保留当前三幕规则，不强制秘密、保密、身份谜题或递送。正文、人物与阻碍由生产 AI 创作，玩家通过正式选项、移动、物品与必要战斗推进。成功必须有规则结算的成功终局事件；若实际生成交付契约，仍须正式完成交付，不能绕过物品规则。

普通主线通过后，交付/退出专项的第一条路线完成委托与显式交付；第二条从同一初态出发，经过至少四次有效行动后明确放弃仍未完成的委托，承担已有规则后果并形成退出终局。不能以终幕 support/challenge 的文字差异冒充先前选择分化，也不以立即退出的一句结尾冒充完整交付故事。两路可以长短不同，不为达到回合数注水。

### 9.2 生产主线与固定初态诊断分开

生产主线登记 claimScope=production_core_story，冻结普通短篇输入与 S1-complete 路线，禁止注入历史 opening 快照。开局和后续生成均走生产入口，响应和每步状态完整记录并严格重放。
...
普通生产主线通过后，再覆盖交付/退出专项、实机创建与重载。不同范围独立记录，不以固定开局成绩替代生产创建，也不以单条通关证明选择分化和整体叙事质量。
...
| 顺序 | 交付 | 完成条件 |
| --- | --- | --- |
| P1-核心主线 | 简化场景编译、规则审阅、真实创建的普通短篇 | 真实 AI 经正式行动到达成功终局，零网络重放匹配状态，能读到完整原文 |
...
普通主线登记一条 complete，固定初态专项登记两路 deliver/withdraw；各最多 24 动作、每 epoch 3 候选/24 HTTP；本批 200 HTTP、90 分钟，超时和取消不退回调用计数。先运行 deliver；未完成则保留 withdraw 为未执行，禁止为完成数立即跑短退出路线。成功后才运行 withdraw。一个注册批次不追加/替换样本；失败先封存与定位，有明确代码根因并完成修复后才能重新冻结验证，不用重复重采替代修复；保留每批结果。
