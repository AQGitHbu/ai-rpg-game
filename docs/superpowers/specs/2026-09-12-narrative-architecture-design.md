...
核心主线登记一条 complete，固定初态专项登记两路 deliver/withdraw；各最多 24 动作、每 epoch 3 候选/24 HTTP；本批 200 HTTP、90 分钟，超时和取消不退回调用计数。先运行 deliver；未完成则保留 withdraw 为未执行，禁止为完成数立即跑短退出路线。成功后才运行 withdraw。一个注册批次不追加/替换样本；失败先封存与定位，有明确代码根因并完成修复后才能重新冻结验证，不用重复重采替代修复；保留每批结果。
