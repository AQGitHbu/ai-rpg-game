...
P1 的建设、收敛及最小收尾证据由原 Plan 和验收报告保留；后续入口为 [P2 较长经历召回与完整中篇 Plan](../plans/2026-09-14-narrative-architecture-p2.md)，其范围依据见 [代码核查](../reports/2026-09-14-narrative-p2-scope-review.md)。P2 按证据链、派生摘要、完整旅程三个里程碑实施，P3/P4 仍为路线图。本文是用户指定的独立任务，不改 `docs/agent/current-phase.json`，不启动旧 Plan5；本次存在重叠的 Thread 能力以本文范围为准。若未来正式替换阶段，届时原子更新阶段配置及对应执行 Plan，不能维护两套执行状态。
...
| P2 | 必需旧事证据链、按视角记忆包、带来源的摘要与短/中篇覆盖 | 按 P2 Plan 完成正式中篇、摘要失败/回放和同状态有无摘要对照；不扩大为新玩法矩阵 |
| P3/P4 | 更丰富世界行为与必要扩容 | 每项能力独立证明游戏性收益，不一次建开放世界 |

原 P1-A/B 的已实现代码与工程证据保留在报告，不重新派发其十项建设任务。P1 Plan 保留收敛与收尾证据，P2 的待执行任务只在其独立 Plan 维护；不维护两份同阶段执行计划。
...
完整路线的叙事质量评价沿用局部衔接、人物动机、因果/悬念、选择后果可感知、结局收束；每维至少 3/5、均分至少 4/5，且无确认硬错误，才标相应叙事质量范围通过。P2 按用户确认的范围以规则约束、召回、调用、上下文组装与恢复的工程正确性完成，具体矩阵见[P2 Plan Task 7](../plans/2026-09-14-narrative-architecture-p2.md#task-7工程封板与-p2-判定当前执行入口)；上述剧情评分与摘要回答收益不阻塞工程完成。漏送必要来源、知识越权、非法引用和状态污染仍是工程缺陷，不归因给模型而豁免。旧批质量与失败结论保持；工程完成不证明优于 main。
