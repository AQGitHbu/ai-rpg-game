...
P3 将自动观察与主动调查显式分开；主动调查只有玩家选择已批准方法后才产生发现和实际见证。NPC 目标依本人有来源的知识、承诺、持有或调查经历结算，并由已有互动及调查前提消费；合作是当前行动的可用性，不另存一份许可状态。需要合作的任务采用明确完成条件，避免两次普通态度回应绕过证据。具体字段、有限绑定、可达性检查与消费者见 P3 Plan Task 1–3；不扩成任意脚本或通用分支图。
