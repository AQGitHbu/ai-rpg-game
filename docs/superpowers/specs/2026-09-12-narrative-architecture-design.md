...
候选修订必须绑定稳定的获准 NPC 方案。仅修正文时复用授权依赖；确需重新判断角色方案时显式更换依赖版本，并使不再适用的旧修订意见失效。先用保存的实际候选验证权限和审阅边界；记录同一文本的缺陷是否延迟到后续版本才出现，不以增加 reviewer、逐句白名单或无限版本解决。这些边界尚待实现，当前完整故事验收不能宣称已通过。
