...
本文是总 Spec，规定本次优化的目标、逻辑契约、分期边界及验收条件；设计不等于已实现，逐文件任务与证据由各阶段 Plan 维护。后续实施入口为 [P3 有后果的调查与角色合作 Plan](../plans/2026-09-15-narrative-architecture-p3.md)。
...
[P1 Plan](../plans/2026-09-12-narrative-architecture-p1.md)保留建设、收敛及最小收尾证据；[P2 Plan](../plans/2026-09-14-narrative-architecture-p2.md)保留证据链、派生摘要与正式旅程任务，其完成依据为[工程验收](../reports/2026-09-15-narrative-p2-engineering-acceptance.md)。P3 根据[最新代码与范围核查](../reports/2026-09-15-narrative-p3-scope-review.md)补齐主动调查、目标结算与合作分化，P4 保留为后续路线图。本文是用户指定的独立任务，不改 `docs/agent/current-phase.json`，不启动旧 Plan5；本次存在重叠的 Thread 能力以本文范围为准。若未来正式替换阶段，届时原子更新阶段配置及对应执行 Plan，不能维护两套执行状态。
...
P1 覆盖可靠披露、怀疑、拒答及信息保留；不把“我不知道”机械写回知识。保存原话和受众，为后续谎言/错误信念留出来源；P3 先让真实调查及实际告知改变角色合作，相互矛盾的信念生命周期另期确认。未进入当前状态且未获创建批准的实质细节仍是候选，不得在正文中先行成立。
...
| NPC 目标 | 复用 goals 与受控状态更新命令 | P3 补齐有来源的完成/阻碍条件及合作消费者；多目标取舍与离场活动另期确认 |
...
P2 实施四类信息文档的初始配置：未摘要原文阈值 50、批大小 10；按 observer 可见、实际已提交且非展示选项的 History 条目计数，可按实际长度提前压缩。有效摘要记录版本、coveredThroughSequence、来源范围和实际保留的实体引用；游标使用 History.sequence，不能用 Event sequence、turn 或 revision 替代。原文永不因摘要删除。摘要失败保留旧覆盖水位，读取全部尚未覆盖原文；超预算不得静默造成历史断档。

使用可重建的分批摘要及概览，避免只能从越来越失真的旧摘要反复压缩。P2 首版采用模型选择原始 History/Event 引用、程序取完整原文的摘录选编；不让模型改写历史中的否定、条件或已发生结果。概览从有效叶摘要对应的原始来源重新选编，精确追问和活跃悬念/承诺仍直接召回原始证据。摘要是否有助于完整叙事由同状态对照和全文验收判断，不能只看压缩率。

摘要以独立、可失效重建的 RPG 缓存保存，按 observer 隔离；不写权威游戏状态、不推进 gameplay revision，不使已展示 token 失效。P2 先在合法生成入口做有界维护，不要求另建后台服务；同一 job 的候选修订固定记忆快照。缓存发布校验来源指纹与独立版本，追加历史、回滚、并发或恢复都不得形成覆盖断档。具体格式、长度政策及预算由 P2 Plan 维护。
...
P3 将自动观察与主动调查显式分开；主动调查只有玩家选择已批准方法后才产生发现和实际见证，普通披露和候选效果也不能跳过它。本期调查限独立 scene 地点，避免把整个 town 当作建筑现场。NPC 目标依本人有来源的知识、承诺、持有或调查经历结算，并由已有互动及调查前提消费；引荐/核验的固定前提与效果范围归 NPC，后续提案不得弱化，合作可用性仍由当前规则推导，不另存一份许可状态。需要合作的任务采用明确完成条件，避免两次普通态度回应绕过证据。具体字段、有限绑定、可达性检查与消费者见 P3 Plan Task 1–3；不扩成任意脚本或通用分支图。
...
P3 继续“提出策略→生成合法选项→玩家确认后结算”，让调查、核验、引荐等已有动作支持新的可行方案。直接自然语言行动解析另期确认；不增加 intent provider，也不让模型批准自己的动作。
...
P3 中若本场实际披露引起目标或任务变化，B 审批预览须同步核对任务、释放、阶段与最终场景选择；本候选缺少必要后续内容时修订或拒绝，不发布后再补状态，不重执行 A，也不扩大生成入口。

### 8.2 未来内容和生成边界

ready 只代表内容可使用，不代表所有未来步骤已发生。新场景停在下一次有效玩家决策或未结算结果之前。实际移动、交付和战斗按原有规则消费，相关历史与认知只在对应步骤真正发生时写入。

P1 保留现有触发与 bundle 消费主干，优先用已确定场景和正式决策覆盖完整短篇；不为阅读导航调用 provider。P3 仅增加两类受控边界：调查结果已结算且缺少有效获批内容；回访时相关状态相对上次实际场景已变化且旧场景不可复用。二者复用同一 source、A/B 和恢复契约，由实际事件证明准入，不能把任意缺稿视为生成许可。地图浏览、普通移动、未变化回访和离场 NPC 行为不因此增加调用。战斗内规则回合继续不等 AI。
...
首个验收故事从正式新游戏创建开始，采用已有规则能够完成的三幕公开递送委托：第一幕明确委托、唯一物品和公开交付依据；第二幕通过知情人物获得与交付有关的路线或处置依据；第三幕抵达绑定接应人，显式交付后作终幕决定。正文、人物、阻碍与具体选择均由生产 AI 创作，不固定开局、不强制秘密或身份谜题，不强制添加无关取物和战斗。中心结果必须由物品归属、成功 item_given 与 ending_reached 共同证明；任务完成不能替代交付，交付不能暗示修船、付款等未执行后果。此范围是首条验收主线，不把所有游戏强制改为递送。

核心主线通过后，交付/退出专项的第一条路线完成委托与显式交付；第二条从同一初态出发，经过至少四次有效行动后明确放弃仍未完成的委托，承担已有规则后果并形成退出终局。不能以终幕 support/challenge 的文字差异冒充先前选择分化，也不以立即退出的一句结尾冒充完整交付故事。两路可以长短不同，不为达到回合数注水。

### 9.2 生产主线与固定初态诊断分开

生产主线登记 claimScope=production_core_story，冻结上述三幕递送短篇输入与 S1-complete 路线，禁止注入历史 opening 快照。开局和后续生成均走生产入口，响应和每步状态完整记录并严格重放。

最小选择对照也可完整严格重放已通过的公开递送故事，在正式创建点和至少四行动后的未交付点备份数据库，再从同一前缀分叉执行真实退出。必须核对原始开局、前缀和完整交付重放状态；明确交付是历史 live、前缀是旧响应重放，新增 live 仅退出分支，不计作两条新生成路线。禁止 JSON 状态注入或截断磁带假称全量重放。

固定初态专项可固定一份此前真实 AI 获批、零行动的 opening SQLite 快照。登记源协议、源代码版本、磁带/审计/数据库哈希及 opening 语义状态；每路复制为独立新数据库，核对初态后才提交玩家行动。原样本只读，不修改旧 binding，也不把不同代码的请求匹配称为 replay。
...
核心生产主线通过后，再覆盖交付/退出专项、实机创建与重载。不同范围独立记录，不以固定开局成绩替代生产创建，也不以单条通关证明选择分化和整体叙事质量。
...
prompt 与 source 编译共用同一投影。续接槽还必须标明正式 trigger 成功后的展示时序和已结算结果，尤其不能用生成时的旧物品归属否定 take/give 场景展示时已经发生的转移；current 槽不得提前消费续接结果。缺槽、多槽、重复槽、未知引用或矛盾绑定拒绝并返回路径；不能丢弃多余正文假装通过。局部新提案可随未发布正文修订，已提交行动和历史不能改变。当前三个候选版本和统一 HTTP 预算保持不变；未增加隐含重试或第二套生产链。

最终幕承担中心冲突的实际处理，而不是把核心行动推给不存在的下一幕。主线 Thread 的既有 Quest 绑定随正式主线延续；任务完成是进入终幕决策的依据，不自动证明中心问题已解决。决策准备与 Thread 关闭分开：具备已满足任务依据且没有未满足额外约束的主线 concern 可进入最终选择，实际结局事件才成为该 concern 的收束依据；未被解决的争议须在结局中明确保留。

P1 的最终选择与结果通过现有叙事包的有界 `endingOutcomes` 表达：仅 trust/doubt 两项，包含具体选择文案与对应结果场景。Action、实际结局 ID 及分支裁决仍由服务端掌握，不增加通用分支图、Entity 字段或互动类型。当前场景停在选择前，两个结果以条件内容经同一次完整作者、审批及审阅批准；未选结果不进入 History、知识或世界事实。正式最终 Action 按实际规则选中的 ending ID 发布对应结果场景，与结局事件及 History 原子提交，保留保密违约等优先规则。缺失或失效结果零写入并显式失败，不用旧场景或确定性结尾冒充。此消费不增加结局后 provider 入口。结果必须对应当前权限与正式行为，不能借条件叙事虚构未发生的交付、开船或不在场人物的协议。

收敛的下一项设计边界是审阅判定契约。正式规则事实（物品归属、可操作对象、行动结果、线索与权限）与不产生玩法效果的场景/服饰描写分开判断；一般细节不自动要求预先登记 Entity，但不得与既有描述冲突、冒充物品获取、增加可用资源或成为未经批准的新线索。对话中的行动意向也不等于规则已结算动作，审阅须核对实际语义，不能只按动作词拒绝。

可引用但不可披露的事实需要独立、正文为空的授权投影，说明来源、允许操作和候选依赖。引用存在性、角色知识与操作授权由确定性审批裁决；语义 review 核对表达、实际受众披露和因果，不从公开正文目录反推秘密不存在。这个投影不是放开秘密，也不是新增长期信息存储类别。

候选修订必须绑定稳定的获准 NPC 方案。仅修正文时复用授权依赖；确需重新判断角色方案时显式更换依赖版本，并使不再适用的旧修订意见失效。先用保存的实际候选验证权限和审阅边界；记录同一文本的缺陷是否延迟到后续版本才出现，不以增加 reviewer、逐句白名单或无限版本解决。实现和验收进度只在 P1 Plan 与报告维护，规则闭环与完整故事结果分开判断。
...
| 顺序 | 交付 | 完成条件 |
| --- | --- | --- |
| P1-核心主线 | 简化场景编译、规则审阅、真实创建的三幕递送短篇 | 真实 AI 经正式行动到达成功终局，零网络重放匹配状态，能读到完整原文 |
| P1-选择分化 | 同一初态的交付/主动退出两路与响应回放 | 世界后果不同且结局承接它们，零网络重放匹配状态，原样本保留 |
| P1-生产确认 | 一次实机创建→完整故事→中途重载→结局 | 确认真实按钮、显示、失败重试和存档链路可用；只修阻断，不做界面打磨 |
| 后续综合验收 | 保密、引荐、核验、长期召回、六路线矩阵、完整 UI 与 main 对照 | P2/P3 相关能力稳定后逐项覆盖；保留失败记录，不作为最小 P1 收尾前置，不据此提前宣称可替换 main |
| P2 | 必需旧事证据链、按视角记忆包、带来源的摘要与短/中篇覆盖 | 按 P2 Plan 的 M1–M7 工程矩阵核对正式请求、权限、摘要、恢复和 UI；全文质量及摘要收益单独报告 |
| P3 | 主动调查、有来源的 NPC 目标结算、合作分化、受控结果生成与回访 | 按 P3 Plan 的 C1–C8 完成两条同源短篇路线、实际 UI 与恢复；文学质量单独报告 |
| P4 及以后 | 经实际用例证明需要的更丰富世界行为与扩容 | 单项确认收益和范围；直接意图、错误信念与离场模拟不作为 P3 前置 |

原 P1-A/B 的已实现代码与工程证据保留在报告，不重新派发其十项建设任务。各阶段的待执行任务与验收状态只在其独立 Plan 维护；P3 分规则闭环、正式操作链和完整小故事三个里程碑，不维护两份同阶段执行计划。
...
P1 核心主线登记一条 complete，固定初态专项登记两路 deliver/withdraw；各最多 24 动作、每 epoch 3 候选/24 HTTP；本批 200 HTTP、90 分钟，超时和取消不退回调用计数。先运行 deliver；未完成则保留 withdraw 为未执行，禁止为完成数立即跑短退出路线。成功后才运行 withdraw。一个注册批次不追加/替换样本；失败先封存与定位，有明确代码根因并完成修复后才能重新冻结验证，不用重复重采替代修复；保留每批结果。

P2 新批的完整短篇、中篇及摘要对照分别登记，维护请求计入总 HTTP；动作/请求/墙钟预算、固定输入和失败分母按 P2 Plan 冻结。P1 旧批的预算与结果不被 P2 协议重新解释。

P3 以一次真实三幕初始化、首次调查决策前的共同状态分叉两条路线，验证不同方法造成的知识与合作后果。预算、真实 UI、回放及 C1–C8 证据按 P3 Plan Task 7–8 冻结；旧质量实验和六路线矩阵不作为本期准入。
...
完整路线的叙事质量评价沿用局部衔接、人物动机、因果/悬念、选择后果可感知、结局收束；每维至少 3/5、均分至少 4/5，且无确认硬错误，才标相应叙事质量范围通过。P2 按用户确认的范围以规则约束、召回、调用、上下文组装与恢复的工程正确性完成，具体矩阵见[P2 Plan Task 7](../plans/2026-09-14-narrative-architecture-p2.md#task-7工程封板与-p2-判定当前执行入口)。P3 以完整通关、真实选择分化、权限及恢复正确等 C1–C8 为完成条件，全文文学质量单独报告。上述剧情评分和摘要回答收益不阻塞 P2 工程或 P3 工程与玩法完成，不以反复修稿补齐阶段。漏送必要来源、知识越权、非法引用和状态污染仍是工程缺陷，不归因给模型而豁免。旧批质量与失败结论保持；阶段完成不证明优于 main。
...
以下是整个优化方向的实现定位，不是当前待新增模块清单。各阶段 Plan 定义该期精确接口和任务，P3 以当前代码核查为基准；所有实现留在 RPG 层，消费已有 foundation API，不修改共同规范或公共 package。
...
新增持久字段在对应阶段 Plan 中统一确定 schema 升级和拒绝旧存档策略，不能沿用 staged 或先前阶段的版本号推定兼容。主仓与两个 worktree 使用独立开发存档，禁止为测试删除用户 main/staged 存档。旧正式存档按当前不迁移原则明确报不支持，不静默补造成新的原文历史。
