...
为现有 DOM 渲染的 UI 引入视觉图片皮肤系统：UI 外壳（面板/对话框/HUD/模态/Toast/按钮）通过 CSS `border-image` 九宫格皮肤换肤，游戏内容视觉位（题材封面/小镇建筑等）通过统一 registry 实现「有图用图、无图回退 SVG」。机制支持任意多套皮肤，v1 仅随附一套通用暗色古典皮肤与程序化占位资产。

### 1.1 范围内

- `src/app/globals.css` 新增皮肤 token 与 `[data-skin]` 作用域九宫格规则
- `src/app/layout.tsx` 根容器挂 `data-skin="classic"`（一个属性）
...
| 内容位图片（语义性实体图） | React 组件 + TS registry | 复用 `adventureVisuals` 白名单查表模式；有图用图、无图/失败回退现有 SVG |
...
src/app/layout.tsx                     微改：根容器挂 data-skin="classic"
...
[data-skin="classic"] .panel {
  ...
  /* 兜底属性保留：border-width/style/color、background、backdrop-filter 不移除 */
}
```
...
| 优先级 | Surface | 挂载 class | 说明 |
|---|---|---|---|
| 必做 | 面板 | `.panel` / `.compact-panel` | Panel 共享原语输出，一处生效全站受益 |
| 必做 | NPC 对话框 | `.npc-dialogue-*` 容器类 | VN 式对话框是视觉焦点 |
| 必做 | HUD | `AdventureHud` 容器类 | |
| 必做 | 模态 | `AdventureOverlay` / `GenerationStatusModal` 容器类 | |
| 必做 | Toast | `.adventure-toast` | |
| 应做 | 按钮 | `InlineButton` 输出 class | hover/disabled 用 CSS filter（brightness/saturate）叠加，不做多状态资产 |
...
3. **兜底属性永不移除**——皮肤规则只叠加 `border-image`，不覆盖 `border-width/style/color` 与 `background`；`border-image` 加载失败时 CSS 原生回退到几何边框，这就是离线降级。
...
- slice 四边等宽：大 surface 逻辑 36px（2x 图 72px），按钮逻辑 24px（2x 图 48px）。
...
| 接入点 | 形态 | 降级 |
|---|---|---|
| 新游戏题材卡封面（`NewGameSetupForm`） | `ContentAssetImage`（DOM `<img>`） | 无图/onError → 现有视觉 |
| 小镇建筑图（`TownMapSvg` 的 `BUILDING_ART`） | 直接调 registry 查表取 URL，塞进 SVG `<image href>` | `<image>` 失败不渲染，底层 `TILE_FILL` 色块天然兜底 |

`BUILDING_ART` 迁入 registry 后 URL 值不变，为等价重构，现有测试断言不受影响。

## 5. 降级与错误处理汇总

| 场景 | 机制 | 层级 |
|---|---|---|
| Chrome 图缺失/404/离线 | CSS 原生：`border-image` 失效自动露出 `border-style/color` + 半透明背景 | 两级（有皮/无皮），零代码 |
...
- `sharedUiContract.test.tsx` 既有断言继续锁定 class contract 不变；`data-skin="classic"` 挂载断言放 app 层测试（`src/app/**`，属性挂在 `layout.tsx` 根容器）。
