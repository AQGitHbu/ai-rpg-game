...
- 7 个题材的 offline 基线均可正常开局并可走到结局（零 AI 通关回归测试覆盖）。
- 新 case 进入 `v2.json` 后离线下拉自动可用，无需手动同步/复制。
...
- v2.json 新增/调整 case 后离线下拉自动可用，无同步负担。
...
- 保留 `developmentPreset === "phase10-journey-v1"` 独占标记语义，**放宽**为允许一个伴随的 `caseId` 字段。
- `caseId` 必须命中服务端白名单（上述 7 个 caseId）；缺失 → 沿用现有 legacy 武侠基线（向后兼容）；命中 → 传给 `createOfflineJourneyGame(caseId)`；未知 → `400`（归入 `UNEXPECTED_FIELDS` 语义）。
...
- 持有 `GENRE_TO_CASE` 映射与 `OFFLINE_CASE_IDS` 白名单（7 个）。
...
- **offline 通关**：offline 下玩家靠纯规则行动（move/talk/investigate/take_item/battle）推进任务图，最终任务完成→结局_1、失败→结局_2。wuxia 的零 AI 完整通关已由 `phase10FullJourney` 证明；其余 6 题材任务拓扑与 wuxia 同构（题材无关），本次新增回归测试补齐端到端证据。
...
- **新增 7 题材零 AI 通关回归**：对 7 条离线基线各驱动 `createGame(offlineDeps)` + `performAction` 规则循环到结局，断言到达 ending、零 `fetch`。复用 `phase10FullJourney`/`runtimeNarrativeJourney` 的旅程 harness 模式。
...
- 数据来源：复用 v2.json + 派生 seed（不复制、不新建 fixture、新 case 自动可用）。
