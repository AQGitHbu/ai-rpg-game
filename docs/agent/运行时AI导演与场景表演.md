...
审核把问题分为 expression、planning、legacy。表达遗漏或额外提问撤销对应单元及传递依赖，保留有效上游；规划预审首次合法语义拒绝原子消耗本周期唯一修复额度并撤销规划、全部依赖表达及发布凭据，再沿剩余请求/尝试预算重做规划；依赖披露后才执行的延后预审同样使用这一额度。planningSemanticRepair 持久记录周期、输入摘要、状态、受控拒绝与结构获批提案锚点：修复必须保留 opening、worldDelta、steps、terminal、observations、actions，只改任务和候选合同并重新通过完整审批。锚点只发给规划器，不进入表达 SafeContext；恢复和发布均核验世界锚点。第二次语义拒绝、缺少新记录的旧语义拒绝均显式失败，不能通过崩溃或改 unit key 获取新额度。现有显式手动重试才进入新周期并清除修复记录。已选历史 label 与合同不符返回 `legacy_dialogue_contract_mismatch`，不能把旧多余问题补进授权；当前 job 重试不能修复已发布历史 label，本实现不提供历史回滚入口。自由输入不冒充历史固定选项，不补造 inquiries。审核反馈只含服务端受控 ID、枚举与问题维度；规划修复另携带原获批提案和场景契约，不转发审核自由文本。
...
审核凭据分别持久记录实际 attempts、最多一次 protocolCorrections 和最多一次 contentRepairs。最终审核最多三次请求，可先纠正协议、再修复表达并复审，也可在表达复审格式错误时使用尚未消耗的协议额度；规划预审的协议纠正额度在整个周期共享；语义重做后的复审继承原计数，最多一次协议纠正加一次语义修复，改摘要或改单元名称也不能恢复额度。协议错误不清除表达，反馈只含服务端受控路径、错误类型和允许键；下一次模型请求实际携带该反馈。网络/API 失败保留 `AI_CALL_FAILED`，uncertain 显式失败，均不触发文案改写。中断请求的未知结果不当作格式错误，但有界恢复会消耗协议恢复额度。同 cycle 重启或摘要变化不重置任何次数；旧非零凭据缺少分类计数时保守视为分类额度已消耗。审核通过凭据绑定版本、cycle、批准投影与实际输出摘要；恢复与发布都重验，输入、风格、合同或正文变化使旧凭据失效。
