...
- 一次逻辑任务（job）分为四类生成职责，并非固定四次 API 调用：`planning` 决定事件、世界变化提案、NPC 回答内容与两个最终玩家回应意图，再由 `narration`、`character`、`choices` 三类表达单元按 DAG 依赖顺序产展示文本。正常路径只有一次整轮规划，表达阶段只润色获批内容；适用任务先审核规划中的对白合同，表达获批后再经 `dialogue_consistency_review` 核对实际文案。规划素材（尤其 `opening.prologue`）**不是最终展示文本**，必须经旁白单元覆盖后才发布。
...
各 stage 的展示职责保持分离。规划先分配互补内容：可观察变化由旁白交代，回答、态度与取舍交给 NPC；同一事实在双方重复须有明确作用，理解背景不自动成为双方必需正文。无新状态时旁白只做极短衔接，不强制复述玩家问题，也不补造环境、动作或进展。必选节拍和 contentFactIds 的覆盖优先，表达器不能为去重自行删除获批内容。开局规划里 `opening.prologue` 与描述是内容素材，最终序幕由 narration 单元覆盖后才发布；不能直接展示 planner 的 `prologue`。旁白 prompt 不含 NPC 台词输出字段；choice prompt 只返回两条 id/label，禁止前缀、效果与 Action。

实际上一轮对白先按焦点 npcId 对齐，再投影到当前场景；未来场景和其他 NPC 不继承。`previousReply` 仅在其事实引用全部属于该视角允许范围时保留；choices 另接上一轮实际选项与已问维度，避免复读和混淆上一句属于谁。这些历史文字不是新知识来源。`SafeContext.stylePolicy` 由已存 setup 的受控人格标签、concise/novel/cinematic 与 normal/dark 构建，进入三类表达 prompt 及缓存摘要，只影响措辞，不扩大事实权限。主角人格只用于玩家刻画；NPC 使用公开身份及受控 delivery，无公开锚点时不据职业猜测经历、能力、隐藏目标或承诺。各类表达角色的内容边界仍优先于风格。
...
`ExpressionTask.inquiries` 为可选 `{factId,aspects}[]`：仅 ask/challenge 可非空，最多 4 个不同 focusFactIds，各含 1–4 个不重复维度。枚举以 `src/game/domain/expressionTask.ts` 为准；该字段只编码提问维度，具体问题对象和措辞含义由 brief 说明。有事实问询时必须编码；征求偏好、行动提议或核实已批准条件可为空，不能仅按 intent 或关键词判定。投影保留全部维度，拒绝未知字段和越权事实，不转发原始 publicIntent。旧任务兼容但不恢复未编码意图。
...
对白一致性审核独立于披露审核：存在 choices、当前结构化提问，或当前话语需要焦点 NPC 回应时必须执行。服务端按场景、说话人和候选编译独立 checks，每项只含自己的获批 brief、intent、inquiries、answers、已选话语及允许事实；回答检查不携带未来候选的问题。候选及历史合同保留安全编译的 prerequisiteFactIds，区分核实已批准条件与新增开放式提问。checkId 和绑定 factId/aspect 的 inquiryId 由服务端生成，模型只能返回 pass/reject/uncertain 及最多 8 条 `{checkId,type,inquiryId}`；不得选择 scope、unitKey 或 candidateId。未知 ID、跨项问题、未问维度的 missing_response 均为协议错误。每个规划检查项最多报告一条足以拒绝的违规，重复 plan checkId 不静默过滤；表达检查可报告多条真实遗漏。inquiryTargets 仅提供报告地址，不代表已提问或授权新知识。输入最多 16,000 Unicode 码点，超限失败而非截断。

规划合同在表达前审核：安全投影的 plan_answer/plan_option 核对原 brief 是否与问题和回答契约一致，不伪造表达结果。普通单元先批量审核；依赖实际上游披露的单元，只能在有效观察来源、DAG 顺序和玩家/说话人权限均支持时延后，待真实输出批准后重新投影并审核。混合条件候选按整个 choices 单元延后，不部分绕过权限。逐单元 planningDialogueReviews 凭据绑定规划摘要，执行对应表达前及发布时都须覆盖；SQLite 发布事务重验凭据。最终审核仅检查实际 answer/option 及 selected 历史合同，不重复审核规划。旧任务可读但不能用缺失或旧政策的凭据发布。

审核把问题分为 expression、planning、legacy。表达遗漏或额外提问撤销对应单元及传递依赖，保留有效上游；规划预审首次合法语义拒绝原子消耗本周期唯一语义修复额度并撤销规划、全部依赖表达及发布凭据，再沿剩余请求/尝试预算重做规划；依赖披露后才执行的延后预审同样使用这一额度。planningSemanticRepair 持久记录周期、输入摘要、状态、受控拒绝与结构获批提案锚点：修复必须保留 opening、worldDelta、steps、terminal、observations、actions，只改任务和候选合同并重新通过完整审批。锚点只发给规划器，不进入表达 SafeContext；恢复、重新审批和发布全部复验锚点、场景图与知识/披露权限。第二次语义拒绝、缺少新记录的旧语义拒绝均显式失败，不能通过崩溃或改 unit key 获取新额度。现有显式手动重试才进入新周期并清除修复记录。已选历史 label 与合同不符返回 `legacy_dialogue_contract_mismatch`，不能把旧多余问题补进授权；当前 job 重试不能修复已发布历史 label，本实现不提供历史回滚入口。自由输入不冒充历史固定选项，不补造 inquiries。审核反馈只含服务端受控 ID、枚举与问题维度；规划修复另携带原获批提案和场景契约，不转发审核自由文本。

基础 source 请求数为 `1 + plan.units.length + 适用时1次规划前置审核 + 适用时1次最终对白审核`；单旁白、单 NPC、一处普通决策且无需延后审核的无重试路径为 6 次，披露审核及延后规划审核另计。每 job 在基础额度外最多 12 次额外 source 请求，表达单元每周期最多 4 次尝试。审核请求每次最多 30 秒、transport 内部最多 1 次；所有 source 请求在发送前持久扣费，真实 HTTP（含 transport 重试）另由 client 审计计数。审核同样受 lease/fence、取消信号和 job 截止时间约束。

审核凭据分别持久记录实际 attempts、最多一次 protocolCorrections 和最多一次 contentRepairs。每个审核单元最多三次请求；批量规划预审和依赖披露后的延后规划预审各自实际发送时另扣 source 请求。最终审核可先纠正协议、再修复表达并复审，也可在表达复审格式错误时使用尚未消耗的协议额度；规划预审的唯一协议纠正额度在整个周期共享，语义重做后的复审继承原计数，因此全周期最多一次规划审核协议纠正和一次规划语义修复，改摘要或改单元名称也不能恢复额度。协议错误不清除表达，反馈只含服务端受控路径、错误类型和允许键；下一次模型请求实际携带该反馈。网络/API 失败保留 `AI_CALL_FAILED`，uncertain 显式失败，均不触发文案改写。中断请求的未知结果不当作格式错误，但有界恢复会消耗协议恢复额度。同 cycle 重启或摘要变化不重置任何次数；旧非零凭据缺少分类计数时保守视为分类额度已消耗。审核通过凭据绑定版本、cycle、批准投影与实际输出摘要；恢复与发布都重验，输入、风格、合同或正文变化使旧凭据失效。
