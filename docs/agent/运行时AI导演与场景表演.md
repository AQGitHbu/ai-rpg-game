...
- 条件终局槽的标签、正文和 `worldDelta.endingPair` 的名称/概要共用 `endingResolutions`：目标 NPC 复用正式终幕选择逻辑，support/challenge 分别通过现有纯规则 `resolveTurn` 预览，提供真实 Action、当前/结果地点、规则事件种类、物品归属变化及新发现事实 ID。`npcLocations` 保留已见或当前同场 active NPC 的真实前后地点；未见的异地 NPC 不进入该投影。前后地点未变不能被写成异地到场或参与关键事项，也不能把待执行前提改为其他 NPC 自行履行；同场合法回应和无关键状态效果的表现仍可创作。预览不写入、不提供事件引用 ID 或秘密正文；新结局对尚未具象化时不伪造定义，`resolvedEndingId=null` 只是该快照的预览。实际结局 ID 决定发布，不能把 trust/doubt 主题反推为已经执行某种立场，两结果不同时发生。中心冲突必须在既有已发生依据与真实行动能力内收束，结局标签/名称不授予额外行动，未履行条件不能靠结果正文兑现。审阅可引用 `ending:trust|doubt` 的 `ruleBasis`，缺陷路径指向对应主题实际 `endingOutcomes` 数组项的 `choiceLabel`/`scene` 或 `worldDelta.endingPair` 数组项的 `name`/`description`；这类依据与主题错绑时整次审阅仍为 uncertain。`worldDelta.beatSummary` 与 currentScene 同为选择前已提交回应，只能按现有 `action:<job.actionId>` 等已发生依据核对，不能引用条件结局依据将待执行结果提前写为事实。其余依据校验保持原契约。
