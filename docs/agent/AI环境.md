...
- 配置诊断码（`AI_CONFIG_*`，来自 `parseAiRuntimeConfig`）：`AI_CONFIG_BASE_URL_MISSING|PLACEHOLDER|INVALID`、`AI_CONFIG_MODEL_MISSING|PLACEHOLDER`、`AI_CONFIG_KEY_MISSING|PLACEHOLDER`；绝不回显任何值。
