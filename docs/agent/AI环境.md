...
- 统一规划对白回归：进程内设置 `RUN_REAL_AI_SINGLE_PLANNER=1` 后运行 `npx vitest run src/game/application/testing/singlePlanner.live.test.ts --maxWorkers=1 --minWorkers=1`；只读本仓 AI 配置，不写游戏数据库，审计与结果位于 `tmp/single-planner-live/`。
