...
- `ai_call.role` 为 `planning`、`narration`、`character`、`choices`（分阶段叙事生产路径的四个 stage）或 `intent`、`opening`、`scene`、`world`、`narrative_bundle`（旧路径，仅 fixture/审计兼容）。每个 stage 恰好一次 provider transport attempt；一次逻辑 event 的完整链路可由同一 `callId` 前缀与 `context` 配对。
