...
### 配置模型恢复后的仙侠 paired regression（2026-08-04）

- 全部请求严格使用 `.env.local` 的 `AI_MODEL=ai-slg-game-model`。同一 captured blueprint 的 explore artifact `artifacts/story-eval/xianxia-a-explore-0-2026-08-04T03-25-41-369Z-7f979252` 完成 18 幕、`fallbackRate=0`，但 `status=max_scenes`，没有 climax/resolution；主线规则推进为 `15/18`，目标事件命中 `8/18`，因此不能当作探索收敛。
- objective 首次复测 `artifacts/story-eval/xianxia-a-objective-1-2026-08-04T03-45-50-231Z-7eda7481` 在首个 director 请求两次 120 秒超时而 `aborted`，不计入质量证据。随后重试 `artifacts/story-eval/xianxia-a-objective-0-2026-08-04T03-52-40-096Z-66ee8cb6` 收敛至 14 幕，主线推进/事实使用与调查均完整，但 3 幕 provider `service_error` fallback，`fallbackRate=3/14=21.4%`；非 release gate 明确失败 `FALLBACK_RATE_HIGH`。
...
同一矩阵的 judge 抽查还发现 `investigate` 行动默认都显示为“调查线索”，多个事实因此在玩家选择和评审证据中看起来是重复选项。行动投影现在对已明确可调查的事实生成 20 字以内的安全预览（完整事实仍要执行调查后才进入已发现事实卡），并保留空文本时的通用回退；相关行动回归测试与 TypeScript/ESLint 已通过。该修复只改变可读性和选择区分度，不改变事实发现规则。
