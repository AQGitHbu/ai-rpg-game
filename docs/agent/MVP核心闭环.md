...
- 2026-07-29：完成 Phase 4B 真实 AI 动态开局（跨仓：foundation `@ai-game/ai-transport@0.1.0` + SLG 迁移 + RPG live source）：生产在有效本项目 AI 配置时使用 live source；配置/服务/候选失败在最多两次尝试与一次机械修复后安全 fallback；fixture 测试仍为主回归；opt-in 真实 smoke（`smoke:ai:phase4b`）已就绪但尚未执行，待人工外部验收。
