...
- support/challenge 的 fallback 文案不是“角色名 → 固定句”映射：服务端使用本局生成种子、幕次、当前地点/目标、行动类型、回合号和该 NPC 已有交互条数选择确定性变体。同一上下文可重放，同一 NPC 的连续正式回合会轮换文案，不同开局也不会机械复用整组对白；live 只返回 NPC 台词时的局部修复必须复用同一上下文 fallback。
