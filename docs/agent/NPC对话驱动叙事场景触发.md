...
- 正式对白提交后进入“等待 NPC 回应”态，pending 期间暂时隐去旧台词；后台写回 ready 快照后，UI 直接回到同一 NPC 的对话框并显示新台词与下一组选项，不插入“继续对话/查看下一步”按钮。若目标变化，HUD/任务面板显示权威下一步，旧焦点对白按交接规则关闭。
...
- NPC 回应必须承接当前玩家话语或当前交谈情境；“我知道了”“好的”“嗯”等无对象确认句，以及“你想问哪一段 / 你还想了解什么”这类只把责任推回玩家的空泛追问，都不是合法的上下文回应，live source 会先带失败原因重试一次，仍不合格才走同一故事上下文的安全 fallback。
- pending 场景上下文必须携带上一句 NPC 原话、玩家上一轮固定选项/自定义回应、`dialogueAct` 与结构化 `topic`；这组历史用于生成本轮 NPC 回应和下一组选项。选项的 `candidateId/action` 仍由服务端锁定，live AI 只能从合法候选中选两个并为其生成直接对白/动作措辞；live prompt 不下发 deterministic fallback 的自然语言 label，只下发动作语义，避免模型照抄上一轮模板。
...
- 确定性 fallback 按当前主线目标和结构化对话状态补足上下文；不再按 NPC 角色名或对白关键词选择固定主题，也不会凭身份制造未出现的地点、时间或证物。NPC 可说事实卡由 live performer 按 `usedFactIds` 约束使用；当本轮台词带有可解析的事实引用时，fallback 先以本轮事实锚定选项，再使用上一轮立场。support/challenge 仍使用本局生成种子、幕次、当前地点/目标、行动类型、回合号和该 NPC 已有交互条数选择确定性选项变体。同一上下文可重放，同一 NPC 的连续正式回合会优先承接上一轮立场；live 复用本轮生成前两个 fallback label 时视为 stale template，先进行一次内容修复；live 只返回 NPC 台词时的局部修复必须复用同一上下文 fallback。
