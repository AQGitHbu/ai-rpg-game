...
| 剧情连续性与结构化记忆 | `agent/剧情连续性与结构化记忆.md` | `策划文档/AI生成RPG_MVP.md` | Plan 4 已实现 v5/v8 committed Event、完整 ledger 重建 Episode/近期场景/NPC contacts、结构化检索与 NPC provenance；Prompt 只接收有界 memory cards，SQLite 校验 memory 漂移；短篇/中篇正式支持，Living Outline/Arc、scene planning、长篇 segmented ledger 与向量索引仍属后续 Plan |
