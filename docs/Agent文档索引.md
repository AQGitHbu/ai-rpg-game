...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | 「实体与组件世界状态基础」正在 `codex/entity-component-world-state-foundation` 执行：Task 1–6 已建立 v3 `EntityStore`、兼容 projector/validator、规则可信 `EntityMutation`、世界演化/候选事件迁移、完整战斗回滚、SQLite 实体状态持久化门禁及受限 `ProposedEntityCommand` parser/approver；command 尚未接入 provider。 |
