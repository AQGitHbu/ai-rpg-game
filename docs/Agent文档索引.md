...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | 「实体与组件世界状态基础」正在 `codex/entity-component-world-state-foundation` 执行：Task 1–5 已建立 v3 `EntityStore`、兼容 projector/validator、规则可信 `EntityMutation`、世界演化/候选事件迁移、完整战斗回滚及 SQLite 实体状态持久化门禁；不可信 `ProposedEntityCommand` 仍待后续任务完成。 |
