...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | 「实体与组件世界状态基础」（Plan 2）已合并 main 并标记 `completed / merged`：包含 v3 `EntityStore`、兼容 projector/validator、规则可信 mutation、严格 SQLite 持久化门禁、受限 `ProposedEntityCommand` 及 Entity 规则闭包 narrative context；command 尚未接入 provider。Plan 3 详细计划见 `superpowers/plans/2026-08-31-npc-personality-knowledge-relationship-graph.md`，尚未启动。 |
