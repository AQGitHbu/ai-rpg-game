...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | v2.1 基础修复 R6 已实现（implemented）：R5 NPC 记忆/知识最小权限与场景审批 + R6 Expansion 生产闭环（live/fixture 源 + 复用 + 审批 + 应用）、Action union 全闭合、15 回合完整旅程、同 seed 分叉与多结局证明、持久化规模边界与长篇门禁；v2.1 为当前生产链，v1 仅作历史参考（分支 `codex-v2-foundation-remediation`，worktree `.worktrees/v2-foundation-remediation`）；唯一阶段 Plan：`superpowers/plans/2026-08-08-ai-rpg-v2-foundation-remediation.md` |
...
| 行动裁决 | `agent/行动裁决.md` | `策划文档/AI生成RPG_MVP.md` | Phase 6 已实现：observe / talk / investigate / move / take_item / battle intents；v2.1（Task 29）从生产 `Action` union 移除无规则实现的 use_item/give_item/interact/accept_quest/narrative_choice，explore→`location_explored`、rest→`player_rested` 无状态行动也产生主事件 |
...
| 运行时 AI 导演与场景表演 | `agent/运行时AI导演与场景表演.md` | `策划文档/AI生成RPG_MVP.md` | Phase 14 P0 已实现；v2.1 SceneSource 只提案，审批后以同一次 CAS 持久化 ready scene、post-writeback revision 的 ApprovedChoice registry 与候选池，客户端场景无 actionKey，live source 只选服务端候选 ID（2026-08-09）；无生图 |
| NPC 对话驱动叙事场景触发 | `agent/NPC对话驱动叙事场景触发.md` | `策划文档/AI生成RPG_MVP.md` | Phase 14 已实现；v2.1 生产链的焦点 NPC 双固定选项与自定义输入统一经 `/api/v2/game/actions` → `performTurn`，浏览器逐次 UUID，短问候也记录回合并创建 pending job；v1 对话端点仅作历史参考（2026-08-09） |
...
| 剧情连续性与结构化记忆 | `agent/剧情连续性与结构化记忆.md` | `策划文档/AI生成RPG_MVP.md` | Phase 11 已实现：规则事件归约有界记忆、主线节奏约束、最小权限连续性上下文与离线 replay；2026-07-31 审计修复后真实 15 调用 journey 与即时 replay 均通过；v2.1（Task 32）明确短篇/中篇正式支持上限与长篇门禁（启用 long/open 前须另立 segmented ledger Plan） |
| 蓝图动态化 | `agent/蓝图动态化.md` | `策划文档/AI生成RPG_MVP.md` §4 | BudgetPolicy 时长档位驱动、主线幕数可变、运行时导演提议地点/NPC/事件级事实物品敌人，经闸门审批后 CAS 持久化；契约 scenario-dynamic-v2 / runtime-narrative-v3；v2.1（Task 27-28）补齐 Expansion 纯规则完整性（trigger/低张力/quest_gap/reuse/收束信号、approve 同批 ID 唯一与跨批引用、apply 解锁/双向连接/挂载索引）并注入 live/fixture ExpansionSource |
