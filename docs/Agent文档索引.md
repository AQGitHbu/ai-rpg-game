...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | Phase 11 已立项（planned / not_started）：剧情连续性与结构化记忆（内容推进引擎）；唯一阶段 Plan：`superpowers/plans/2026-07-31-mvp-phase-11-story-continuity-memory.md` |
...
| 运行时 AI 导演与场景表演 | `agent/运行时AI导演与场景表演.md` | `策划文档/AI生成RPG_MVP.md` | Phase 10 已收尾：真实三请求最小权限链路、两个服务端批准选项、三次同角色有界尝试、机械 schema/引用收敛、完整 fallback 和双模式完整旅程；就绪 town 地点注入 `townSpatial` 空间语义上下文（不含坐标，2026-07-31）；无自由输入、运行时蓝图扩容或生图 |
| 剧情连续性与结构化记忆 | `agent/剧情连续性与结构化记忆.md` | `策划文档/AI生成RPG_MVP.md` | Phase 11 planned：以规则事件归约有界记忆、主线节奏约束和最小权限连续性上下文；尚未实现 |
