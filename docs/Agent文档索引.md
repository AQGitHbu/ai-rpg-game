...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | 「NPC 人格、知识与关系图」（Plan 3）待执行：目标是把 NPC 人格锚点、动态状态、知识来源、交互历史、承诺与有向关系边做成唯一权威 Entity Component，统一供对话/调查/物品/任务/战斗读取，并把 `WorldState` 升到 v4、`EntityStore` 升到 v2；明确排除 Plan 4 的 Event/Episode 与稳定 `eventId`、Plan 5 的 Living Outline/Arc、长篇分段存储、Identity Anchors 运行期更新与任何新增 provider 调用。Plan 3 的组件、关系规则、知识来源、speech authority 与 continuity journey 目前均不存在。上一阶段「实体与组件世界状态基础」（Plan 2）已合并 main：v3 `EntityStore`、兼容 projector/validator、规则可信 mutation、严格 SQLite 持久化门禁与受限 `ProposedEntityCommand`（command 尚未接入 provider）。Plan 见 `superpowers/plans/2026-08-31-npc-personality-knowledge-relationship-graph.md` |
