...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | Phase 10：运行时 AI 导演与场景表演已设计、待实现（planned / not_started，单仓 `codex/phase10-runtime-ai-director`）；唯一 Plan：`superpowers/plans/2026-07-30-mvp-phase-10-runtime-ai-director.md`；Spec：`superpowers/specs/2026-07-30-runtime-ai-director-scene-performance-design.md` |
...
| 运行时 AI 导演与场景表演 | `agent/运行时AI导演与场景表演.md` | `策划文档/AI生成RPG_MVP.md` | Phase 10 已设计、待实现：director / writer / npc 三请求最小权限链路、两个服务端批准选项、完整 fallback；无自由输入、运行时蓝图扩容或生图 |
...
| AI 环境 | `agent/AI环境.md` | `.env.example` | 独立环境契约已建立；Phase 4B/4C 已实现开局 AI、structured output 与 opt-in smoke；Phase 10 计划复用同一 transport/config，增加 director / writer / npc 三个独立 runtime 请求与 `RUN_REAL_AI_RUNTIME_SMOKE=1` 门禁；`GAME_DB_PATH` 仍为 server-only |
