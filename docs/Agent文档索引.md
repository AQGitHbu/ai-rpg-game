...
| 战斗与结局 | `agent/战斗与结局.md` | `策划文档/AI生成RPG_MVP.md` | 已实现规则拥有五项战斗属性、1–2 普通敌人/Boss 遭遇、速度队列、多玩家单位接口、目标绑定 opaque controls、active fast path 与多单位战斗视图；HP 为 0 的单位从当前战斗画面移除（2026-08-13） |
| 地图与地点冒险 | `agent/地图与地点冒险.md` | `策划文档/AI生成RPG_MVP.md` | Phase 7 地图/地点主循环、Phase 8 非战斗 HUD/视窗/行动栏/详情/对话层、响应式全屏地图 HUD 细化均已实现；Phase 10 narrative 仅在进入地点后显示，地图始终为入口；town 层已接入主循环（map→town→scene 三层导航，2026-07-31）；序幕阅读期间静默预生成且确认不再启动 AI，普通 pending 不以全屏模态打断（2026-08-13）；TownView 对旧存档的在场 NPC/slot 失同步提供只读补偿（2026-08-12）；建筑入口只导航、明确 talk 行动只打开预生成对话，正式回合由双选项/自定义输入提交（2026-08-13）；fallback 对话投影固定双选项；无真实行动时显示异常恢复提示（2026-08-13）；NPC 对话框统一显示直接台词并兼容旧存档叙述前缀；新增 NPC 不自动弹窗，目标 ready 后显示权威“下一步”提示（2026-08-13）；建筑场景使用具体建筑名与绑定 NPC 上下文，行动栏按当前幕 opaque 目标 token 收口（2026-08-13） |
...
| NPC 对话驱动叙事场景触发 | `agent/NPC对话驱动叙事场景触发.md` | `策划文档/AI生成RPG_MVP.md` | 焦点 NPC 始终提供两个固定选择与一个自定义输入；两者统一经 `/api/game/actions` → `performTurn`，浏览器逐次 UUID，均记录回合并创建 pending job；建筑与 talk 入口不提交，只有对话选项/自定义输入提交并立即后台生成（2026-08-13）；NPC 回应只保留第一人称直接台词，必须承接当前话语或情境；幕交接关闭旧弹窗、旧 NPC 退出焦点，read model 可修复旧存档的冲突焦点（2026-08-13） |
