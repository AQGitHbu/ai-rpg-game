...
| 探索与任务推进 | `agent/探索与任务推进.md` | `策划文档/AI生成RPG_MVP.md` | Phase 6 已实现：移动、调查、任务解锁、obtain_item / defeat_enemy objective 与 outcome；探索/调查入口按当前地点事实与实体投影（`hasExplorableContent`，2026-08-12）；2026-08-19：调查/移动即时场景写回，优先消费对话回合 AI 预生成 `linearNarrativeQueue` 叙事，未命中确定性兜底（结构化下一目标动线提示 + `linear_narrative_fallback` 失败码）；application server composition root 已将 investigate 纳入同步场景完成分流，并由回归测试守护 |
