...
| 当前开发阶段 | `agent/当前开发阶段.md` | `agent/current-phase.json` | 「实体与组件世界状态基础」正在 `codex/entity-component-world-state-foundation` 执行：Task 1–3 已建立 v3 `EntityStore`、兼容 projector/validator 与规则可信 `EntityMutation` 的基础行动迁移；世界演化、战斗检查点和不可信 `ProposedEntityCommand` 仍待后续任务完成。 |
