...
import { mkdtempSync, readFileSync, cpSync, writeFileSync, existsSync, unlinkSync } from 'node:fs';
...
test('actual stage A completes formal policy and strictly replays terminal evidence', { timeout: 600000 }, async () => {
  ...
  assert.deepEqual((await p2BatchStatus(protocol, join(root, 'live'))).stages, { A: 'awaiting_review', B: 'not_executed' });
  ...
  assert.deepEqual((await p2BatchStatus(protocol, failedDirectory)).stages, { A: 'sealed_fail', B: 'not_executed' });
  ...
  assert.deepEqual((await p2BatchStatus(protocol, join(root, 'live'))), { plannedRoutes: 2, stages: { A: 'sealed_pass', B: 'not_executed' }, sealedPassRoutes: 1, passed: false });
  const bPath = join(root, 'live', 'B.route-manifest.json');
  writeFileSync(bPath, JSON.stringify({ state: { status: 'sealed_pass' } }));
  assert.deepEqual(await p2BatchStatus(protocol, join(root, 'live')), { plannedRoutes: 2, stages: { A: 'sealed_pass', B: 'invalid' }, sealedPassRoutes: 1, passed: false });
  unlinkSync(bPath);
  const b = createP2RouteManifest({ directory: join(root, 'live'), stage: 'B', budget: protocol.routes[1].budget,
    binding: { ...inspection.head.binding, sourceHash: hashReplayValue({ stage: 'B', input: protocol.input }) } });
  const originalB = readFileSync(bPath, 'utf8');
  const tamperedB = JSON.parse(originalB); tamperedB.state.status = 'sealed_pass';
  writeFileSync(bPath, JSON.stringify(tamperedB));
  assert.equal((await p2BatchStatus(protocol, join(root, 'live'))).stages.B, 'invalid');
  writeFileSync(bPath, originalB);
  b.advance(p2ManifestGuard(b.read()), { type: 'start' });
  b.advance(p2ManifestGuard(b.read()), { type: 'pause', status: 'awaiting_review', reason: 'terminal_quality', artifacts: { fake: 'a'.repeat(64) } });
  b.advance(p2ManifestGuard(b.read()), { type: 'seal_pass', artifacts: { 'terminal-review': 'b'.repeat(64), 'terminal-quality': 'c'.repeat(64) } });
  assert.deepEqual(await p2BatchStatus(protocol, join(root, 'live')), { plannedRoutes: 2, stages: { A: 'sealed_pass', B: 'invalid' }, sealedPassRoutes: 1, passed: false });
  ...
  assert.equal((await p2BatchStatus(protocol, join(root, 'live'))).sealedPassRoutes, 0);
