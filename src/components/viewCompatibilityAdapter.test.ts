...
function makeCurrentView(overrides: {
...
describe("adaptToCompatibleView dialogues", () => {
  ...
  it("falls back to narrative.npcLine when npcDialogues is absent", () => {
    const v = makeCurrentView({ npcLine: { npcId: "npc_1", text: "直接台词。", emotion: "neutral" } });
    const adapted = adaptToCompatibleView(v) as unknown as {
  ...
  it("chain: deterministic scene -> projection -> adapter keeps every present NPC voiced", async () => {
    ...
    const adapted = adaptToCompatibleView(view) as unknown as {
      dialogues: readonly { npcId: string; speechPages: readonly string[] }[];
    };
