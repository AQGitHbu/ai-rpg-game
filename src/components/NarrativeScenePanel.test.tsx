...
describe("NarrativeScenePanel", () => {
  it("shows only player-safe text and submits the opaque token", async () => {
    ...
    render(<NarrativeScenePanel busy={false} onChoose={onChoose} onReturnMap={vi.fn()} scene={{ narration: "雨落在屋檐上。", npcLine: null, choices: [{ label: "继续追问", choiceToken: "a" }, { label: "检查角落", choiceToken: "b" }], npcDialogues: [] }} />);
  ...
  it("returns to the map locally without exposing narrative internals", async () => {
    ...
    render(<NarrativeScenePanel busy={false} onChoose={vi.fn()} onReturnMap={onReturnMap} scene={{ narration: "雨落在屋檐上。", npcLine: null, choices: [{ label: "继续追问", choiceToken: "a" }, { label: "检查角落", choiceToken: "b" }], npcDialogues: [] }} />);
