...
import { Panel, InlineButton } from "@ai-game/ui";
...
export function LocationSceneScreen({
  ...
  return (
    <Panel
      eyebrow="地点场景"
      header={<h2>{scene.title}</h2>}
    >
      <div className="location-scene" role="group" aria-label={`地点场景：${scene.title}`}>
        <AdventureVisual gameType={gameType} kind={scene.backdrop} label={scene.title} decorative />
        <p>{scene.description}</p>

        <div className="scene-hotspots" role="group" aria-label="场景互动">
        ...
        <InlineButton onClick={onReturnMap}>返回地图</InlineButton>
      </div>
    </Panel>
  );
}
