...
vi.mock("@/game/application/server/compositionRootV2", () => ({
  getServerGameV2EntryPoints: () => mocks,
}));
...
function request(body: unknown): Request {
  return new Request("http://localhost/api/v2/game/actions", {
    method: "POST",
...
beforeEach(() => {
  vi.clearAllMocks();
  mocks.performTurnV2.mockResolvedValue({ ok: true, revision: 8, feedback: "Action performed", view: { revision: 8 } });
});

describe("POST /api/v2/game/actions", () => {
  it("fixed_choice 也进入同一个 performTurnV2 入口", async () => {
    ...
    expect(mocks.performTurnV2).toHaveBeenCalledWith({
      actionId: ACTION_ID,
      interaction: { kind: "fixed_choice", choiceToken: "opaque-token" },
      expectedRevision: 7,
    });
  ...
  it("free_text 直接进入 composition root 的 performTurnV2 并返回统一 view", async () => {
    const response = await POST(request({
      actionId: ACTION_ID,
  ...
  it.each([
    ...
    expect(mocks.performTurnV2).not.toHaveBeenCalled();
  });
});
