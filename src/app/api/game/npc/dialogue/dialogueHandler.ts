...
export async function handleNpcDialogueRequest(
  ...
  if (typeof npcId !== "string" || npcId.length === 0) {
    return json(400, { code: "INVALID_INTENT", detail: "npcId 必须是非空字符串" });
  }

  // 校验 text：拒绝空白输入（分类器的短文本规则只处理有内容的短句）。
  const text = body["text"];
  if (typeof text !== "string" || text.trim().length === 0) {
    return json(400, { code: "INVALID_INTENT", detail: "text 必须是非空字符串" });
  }
  ...
  if (result.ok) {
    if (result.kind === "chat") {
      return json(200, { kind: result.kind, npcSpeech: result.npcSpeech, view: result.view });
    }
    return json(200, { kind: result.kind, view: result.view });
  }
