...
export function updateStoryMetrics(prev: StoryState, newDrafts: readonly NarrativeEventDraft[]): StoryState {
  ...
  for (const draft of newDrafts) {
    ...
    switch (p.type) {
      ...
      case "fact_discovered":
        tension += TENSION_CHANGES.fact_discovered + (p.tensionDelta ?? 0);
        break;
