...
describe("MaterializedView", () => {
  ...
  it("extracts quest_completed as a beat", () => {
    const events: CommittedNarrativeEvent[] = [
      makeCommittedEvent({ type: "quest_completed", questId: asQuestId("q1") } as unknown as NarrativeEventPayload),
    ];
    const v = createEmptyMaterializedView();
  ...
  it("npc_met records contact location (from currentLocationId at commit time)", () => {
    const events: CommittedNarrativeEvent[] = [
      makeCommittedEvent({ type: "npc_met", npcId: asNpcId("npc_1") } as unknown as NarrativeEventPayload),
    ];
    const v = createEmptyMaterializedView();
    const result = reconcileMaterializedView(v, events, asLocationId("loc_1"));
  ...
  it("incremental: only processes new events since cursor", () => {
    const events: CommittedNarrativeEvent[] = [
      makeCommittedEvent({ type: "quest_completed", questId: asQuestId("q1") } as unknown as NarrativeEventPayload),
      makeCommittedEvent({ type: "location_visited", locationId: asLocationId("loc1") } as unknown as NarrativeEventPayload),
    ];
    const v = createEmptyMaterializedView();
...
describe("MaterializedView candidate reaction visibility (Task 20)", () => {
  it("approved candidate reaction (fact_discovered) appears in recentBeats without AI text", () => {
    const events: CommittedNarrativeEvent[] = [
      makeCommittedEvent({ type: "candidate_event_approved", candidateId: "ce-1", kind: "npc_reveals_fact", approvedAtTurn: 4 } as unknown as NarrativeEventPayload),
      makeCommittedEvent({ type: "fact_discovered", factId: asFactId("fact_2") } as unknown as NarrativeEventPayload),
      makeCommittedEvent({ type: "candidate_event_activated", candidateId: "ce-1", kind: "npc_reveals_fact", activatedAtTurn: 4 } as unknown as NarrativeEventPayload),
    ];
    const v = createEmptyMaterializedView();
    const result = reconcileMaterializedView(v, events, asLocationId("loc_1"));
    ...
    // 审计事件（approved/activated）不成为 beat，也不携带 AI 原文
