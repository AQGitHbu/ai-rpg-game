...
/** Shared, pure target selection for NPC deliberation and memory preparation. */
export function selectNpcDeliberationTarget(
  context: NarrativeBundleSourceContext,
): NpcId | undefined {
  if (context.kind !== "decision") return undefined;
  const focus = context.job.focusNpcId;
  if (focus === undefined) return undefined;
  const npc = entitiesOfKind(context.worldState.entityStore, "npc")
    .find((record) => record.core.id === focus);
  if (npc === undefined || npc.core.lifecycle !== "active"
    || npc.position.locationId !== context.worldState.currentLocationId) return undefined;
  ...
  return greetingOnly || !needsJudgment ? undefined : focus;
}
...
export async function prepareNpcNarrativeContext(
  ...
  privateMemory?: NarrativeMemoryContext,
  ...
  const focus = selectNpcDeliberationTarget(context);
  ...
  if (npc === undefined) return { ok: true, context };
  if (privateMemory !== undefined && String(privateMemory.observerId) !== String(focus)) {
    return fail("invalid_reference", "npc_memory_observer_mismatch");
  }
  try {
    const input = projectNpcDeliberation({
      ...
      ...(privateMemory === undefined ? {} : { memoryContext: privateMemory }),
    });
