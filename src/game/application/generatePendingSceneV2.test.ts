...
function makeDeps(record: GameRecordV2 | null, source: SceneSource) {
  return {
...
describe("generatePendingSceneV2", () => {
  it("returns not_pending when generation is idle", async () => {
    ...
    const result = await generatePendingSceneV2(makeDeps(record, spy.source));
  ...
  it("returns unavailable when no active game", async () => {
    const spy = makeSpySceneSource();
    const result = await generatePendingSceneV2(makeDeps(null, spy.source));
  ...
  it("returns unavailable when the scene source fails", async () => {
    ...
    const result = await generatePendingSceneV2(makeDeps(record, failingSceneSource()));
    expect(result).toBe("unavailable");
  });
  ...
  it("process restart: re-hydrating the record from the same JSON rebuilds an identical source context", async () => {
    ...
    const run = async () => {
      const hydrated = JSON.parse(recordJson) as GameRecordV2;
      const spy = makeSpySceneSource();
      const result = await generatePendingSceneV2(makeDeps(hydrated, spy.source));
      expect(result).toBe("saved");
      return spy.contexts();
    };
