...
export const NARRATIVE_P3_INPUT = Object.freeze({
  ...
  storyOpening: "我接下旧契与证人的委托。请先让我从现场提供的合法方法中选择查验方式，再根据实际证据决定是否告知、核验并完成交付。本协议要求开局或第一幕推进实际创建一个未发现、可主动调查的事实，并用 consequenceBindings.bind_investigation（discoveryMode=investigation）激活恰好 2–3 个合法方法；仅写 investigationApproaches 或使用 automatic 事实不满足。",
