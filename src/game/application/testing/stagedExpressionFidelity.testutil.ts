...
export function expressionContext(stage: Unit["stage"], facts: readonly string[]): SafeContext {
  ...
  const parts = visibleFacts.map(fact => ({ text: fact.text, facts: [{ factId: fact.id, certainty: fact.certainty }], evidence: [], beatIds: [] }));
  const draft: UnitOutput = stage === "choices" ? { stage, labels: [{ candidateId: "ask_marks", label: "这些脚印朝哪边去？深浅如何？" }, { candidateId: "challenge_list", label: "这份名单的可信程度如何？" }] }
    : stage === "character" ? { stage, speakerId: "npc_0", parts, answeredBeatIds: [], emotion: "neutral", actions: [] }
    : { stage, parts, actionKeys: [] };
...
export function identityInquiryContext(): SafeContext {
  ...
  return { ...context,
    ...
    options: [
      { candidateId: "ask_marks", dialogueAct: "ask", publicIntent: { text: "这些脚印朝哪边去？深浅如何？", facts: [{ factId: "fact_0", certainty: "known" }], evidence: [], beatIds: [] } },
      { candidateId: "challenge_list", dialogueAct: "challenge", publicIntent: { text: "这份名单的可信程度如何？", facts: [{ factId: "fact_1", certainty: "known" }], evidence: [], beatIds: [] } },
    ],
  };
}
...
export async function controlledDisclosureHarness(text: string) {
  ...
  h.source.generate = async (request, execution) => {
    ...
    if (response.stage === "planning") return { ...response, value: { ...response.value,
      ...
      units: response.value.units.map(u => u.key === "character_npc_0" ? { ...u, taskFactIds: [factId], requiredObservationKeys: ["heard_new"],
        draft: { stage: "character", speakerId: "npc_0", parts: [{ text, facts: [{ factId, certainty: "known" }], evidence: [], beatIds: [] }], answeredBeatIds: [], emotion: "neutral", actions: [] } } : u),
    } };
