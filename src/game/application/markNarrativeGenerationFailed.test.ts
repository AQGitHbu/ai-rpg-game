...
function fixture(): { record: GameRecord; repository: GameRepository } {
  ...
  const repository: GameRepository = {
    ...
    applyState: async (input) => {
      ...
      if (input.expectedRevision !== current.revision || expected === undefined || generation.status === "idle") {
        return { ok: false, code: "STALE_GAME_REVISION" };
      }
      if (generation.status !== expected.status || String(generation.job.jobId) !== expected.jobId) {
        return { ok: false, code: "STALE_GAME_REVISION" };
      }
...
describe("markNarrativeGenerationFailed", () => {
  it("persists only the stable scene failure and keeps revision/world state", async () => {
    ...
    expect(current.ok && current.status === "active" ? current.record.storyState.narrative.generation : null).toMatchObject({
      status: "failed", job: { jobId: "job_failed" }, failure: { kind: "AI_RESPONSE_INVALID", phase: "scene" },
    });
  });
});
