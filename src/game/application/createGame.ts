...
export type OpeningGenerationSource = {
  generate(input: {
    ...
    setup?: GameSetup;
  }): Promise<OpeningGenerationCandidate>;
};
...
// 不生成任何未来命名实体。同 seed+gameType 完全可重放；玩家配置为权威输入。
// ---------------------------------------------------------------------------
    ...
    async generate(input) {
      ...
      return {
        world: {
          ...
          themes: ["探索", "抉择"],
          publicFacts: [
            { key: "fact_inn", text: `${variant.npc}守着通往${variant.route}的消息。` },
            { key: "fact_pact", text: `${variant.clue}与${variant.relic}共同指向${variant.boss}的隐秘计划。` },
          ],
        },
        player: {
          name: setup?.characterName ?? "无名旅者",
          identity: setup?.characterIdentity ?? profile.identity,
          backgroundSummary: setup?.characterProfile ?? `为追寻一段被掩埋的${profile.genre}真相独自上路。`,
          baseStats: { hp: 100, attack: 10, defense: 5 },
        },
        prologue: `你在${profile.start}醒来，窗外的${variant.route}方向传来异动。`,
        storyContract: {
          version: 1,
          targetActs: input.gameLength === "short" ? 3 : 5,
          centralConflict: `旧案背后的${variant.boss}正在瓦解${profile.genre}世界的秩序。`,
          endingDirections: [
        ...
        opening: {
          location: { name: profile.start, description: `一处临近${variant.route}的落脚点。`, scale: "town" },
          npc: {
            name: variant.npc, role: "关键线人",
            description: `掌握${variant.route}沿途消息的知情人。`,
            knownFactKeys: ["fact_inn"], privateFactKeys: ["fact_pact"], goals: ["查明幕后势力"],
          },
          quest: {
            name: `取得${variant.npc}的信任`, description: "从关键线人口中确认追索方向。",
