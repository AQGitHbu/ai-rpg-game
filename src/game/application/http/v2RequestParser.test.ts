...
import { parseV2ActionRequest, httpStatusForV2Code } from "./v2RequestParser";
...
describe("parseV2ActionRequest — fixed_choice", () => {
  it("合法 fixed_choice：只含 kind + choiceToken", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "fixed_choice", choiceToken: "tok" } });
  ...
  it("fixed_choice 拒绝空 token", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "fixed_choice", choiceToken: "" } });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe("INVALID_INPUT");
  });

  it("fixed_choice 拒绝未知字段（如 text）", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "fixed_choice", choiceToken: "tok", text: "hack" } });
    expect(r.ok).toBe(false);
  });
});
...
describe("parseV2ActionRequest — free_text", () => {
  it("合法 free_text：kind + text + targetNpcId", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "free_text", text: "我相信你", targetNpcId: "npc_1" } });
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  it("free_text 必须提供 targetNpcId", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "free_text", text: "休息一下" } });
    expect(r.ok).toBe(false);
  });

  it("free_text 拒绝空文本", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "free_text", text: "", targetNpcId: "npc_1" } });
    expect(r.ok).toBe(false);
  });

  it("free_text 拒绝超长文本（引用统一常量）", () => {
    const tooLong = "田".repeat(PLAYER_UTTERANCE_MAX_LENGTH + 1);
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "free_text", text: tooLong, targetNpcId: "npc_1" } });
    expect(r.ok).toBe(false);
  });

  it("free_text 拒绝非法 targetNpcId（非字符串）", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "free_text", text: "嗨", targetNpcId: 123 } });
    expect(r.ok).toBe(false);
  });
});
...
describe("parseV2ActionRequest — 顶层字段", () => {
  it("拒绝未知 kind", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 0, interaction: { kind: "teleport" } });
    expect(r.ok).toBe(false);
  });

  it("拒绝负数 expectedRevision", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: -1, interaction: { kind: "fixed_choice", choiceToken: "tok" } });
    expect(r.ok).toBe(false);
  });

  it("拒绝小数 expectedRevision", () => {
    const r = parseV2ActionRequest({ actionId: ACTION_ID, expectedRevision: 1.5, interaction: { kind: "fixed_choice", choiceToken: "tok" } });
    expect(r.ok).toBe(false);
  });
  ...
  it("拒绝缺失 actionId / expectedRevision", () => {
    ...
    expect(parseV2ActionRequest({ actionId: ACTION_ID, interaction: { kind: "fixed_choice", choiceToken: "tok" } }).ok).toBe(false);
  });

  it("拒绝非 UUID actionId", () => {
    expect(parseV2ActionRequest({ actionId: "a1", expectedRevision: 0, interaction: { kind: "fixed_choice", choiceToken: "tok" } }).ok).toBe(false);
  });
