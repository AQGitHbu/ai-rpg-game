...
describe("narrative request client", () => {
  it("maps each purpose to the narrative bundle role and purpose-specific policy", async () => {
    ...
    expect(complete).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ policyOverride: expect.objectContaining({ timeoutMs: 240_000, maxAttempts: 2, thinking: "on", reasoningEffort: "low", jsonMode: "prompt_only", maxTokens: undefined }) }),
    );
    expect(complete).toHaveBeenNthCalledWith(
      2,
      "narrative_bundle",
