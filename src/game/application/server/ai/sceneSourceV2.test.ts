...
function makeRecord(pool: readonly EventCandidate[]): GameRecordV2 {
...
function makeRepo(record: GameRecordV2 | null): GameRepositoryV2 {
  let current = record;
  return {
...
describe("sceneSourceV2 候选事件提议 → 写回审批（Task 21）", () => {
  it("合法候选经 generatePendingSceneV2 写回池，且不改变 World State/tension", async () => {
    const record = makeRecord([]);
    const repo = makeRepo(record);
    const source = makeSceneSource([validCandidate("ce-1")]);
    const result = await generatePendingSceneV2({ repository: repo, sceneSource: source, now: () => "2026-01-02" });
    expect(result).toBe("saved");
    const current: GetCurrentGameV2Result = await repo.getCurrentGame();
