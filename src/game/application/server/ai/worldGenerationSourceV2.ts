...
export type WorldGenerationSourceV2Deps = {
...
export function createWorldGenerationSourceV2(
  deps: WorldGenerationSourceV2Deps,
    ...
    async generate(input) {
      ...
      try {
        ...
        if (repaired.candidate === null) {
          logger?.warn("v2_world_generation_repair_failed");
          return fixture.generate(input);
        }

        const validated = validateWorldGenerationCandidate(repaired.candidate);
        if (!validated.ok) {
          logger?.warn("v2_world_generation_validation_failed");
          return fixture.generate(input);
        }
        return validated.validated;
      } catch (error) {
        logger?.warn("v2_world_generation_transport_failed", { message: (error as Error)?.message });
        return fixture.generate(input);
      }
    },
  };
