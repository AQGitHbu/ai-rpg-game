...
function messages(role: Role, request: Request): readonly AiMessage[] {
  ...
  const promptInstruction = role === "director"
    ? `${instruction} If pacing is climax, require sceneGoal to name the established clue, NPC lead, or threat that makes the confrontation inevitable; do not introduce an unforeshadowed antagonist or unsupported final reveal.`
    : instruction;
