...
${INQUIRY_SEMANTICS}
每条inquiries严格绑定factId/aspect/inquiryId。两个fact的相同aspect仍是不同问题。inquiryTargets仅为报告额外提问提供地址，不表示这些维度已经被问过，也不授权任何知识。missing_response只能引用本项inquiries，不能引用未问维度。extra_inquiry仅用于option/plan_option/selected，只能引用本项inquiryTargets里不属于inquiries的地址；answer/plan_answer禁止extra_inquiry，NPC新增未获批话语用intent_mismatch核对brief/intent忠实度。NPC漏答仍用missing_response，回答结果不符仍用answer_mismatch；没有可确定的合法地址时uncertain，不得自造ID。
...
玩家候选或历史选择的直接、间接或反问只要实质索取信息，ask/challenge都必须由inquiries覆盖；空inquiries不授权任意提问。例：challenge且inquiries=[]，brief问“你是不想拦，还是拦不住”，这是计划多问；不确定该fact/aspect就uncertain。reliability只问真假可信程度，“从哪儿听来”新增source；“来源并不重要，我只关心真假”是陈述。礼貌语不新增维度，不得只按关键词判定。
