...
import {
  createRuleIntentParserV2,
  createLiveIntentParserV2,
  createV2IntentParserSource,
  classifyDialogueAct,
  parseIntentPayload,
  type LiveIntentTransport,
} from "./liveIntentParserSourceV2";
...
// ---------------------------------------------------------------------------
// Task 9：live/fixture IntentParserSource V2
...
describe("createRuleIntentParserV2（无 AI 配置时的确定性源）", () => {
  const source = createRuleIntentParserV2();
...
describe("createLiveIntentParserV2（AI 配置有效时的 live 源）", () => {

  it("有效 JSON → dialogueAct 正确映射", async () => {
    const live = createLiveIntentParserV2(stubTransport({ ok: true, content: '{"dialogueAct":"support"}' }));
  ...
  it("非法 JSON → 规则降级；规则可归类时返回规则结果", async () => {
    const live = createLiveIntentParserV2(stubTransport({ ok: true, content: "not a json" }));
    const result = await live.parseIntent("我相信你", ctx, asNpcId("npc_1"));
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it("AI 超时（transport 返回失败）→ ok=false 兜底 result，绝不抛出", async () => {
    const live = createLiveIntentParserV2(stubTransport({ ok: false, code: "timeout" }));
...
it("AI 抛异常 → 规则降级，绝不炸穿", async () => {
    ...
    const live = createLiveIntentParserV2(exploding);
    const result = await live.parseIntent("我相信你", ctx, asNpcId("npc_1"));
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.action).toMatchObject({ type: "talk", dialogueAct: "support" });
...
describe("createV2IntentParserSource（composition root 工厂）", () => {
  it("无 AI 配置 → 确定性 rule 源", () => {
    const source = createV2IntentParserSource({});
    expect(source.sourceVersion).toBe("rule-intent-v2");
  });
  ...
  it("AI 配置有效且注入 transport → live 源", () => {
    ...
    expect(source.sourceVersion).toBe("live-intent-v2");
  });
});