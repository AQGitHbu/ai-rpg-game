...
export function buildDecisionNarrativeContextBlocks(
  ...
  const evolutionRequirement = structuralEvolutionNeed.kind === "next_act"
    ? `本回合已进入第 ${storyState.currentAct} 幕：worldDelta 绝不能为 null，必须提供 newLocation、newNpc、newItem、newEnemy、nextMainQuest；其余字段可为 null。${storyState.currentAct >= storyState.targetActs ? `这是最终幕：新地点、新 NPC、nextMainQuest 及抵达场景必须共同承接中心冲突“${storyState.contract.centralConflict}”的实际处理，现有目标必须在本幕可执行完成，不能继续引荐或转交给不存在的下一幕；当前只建立终幕处理场景与任务，不提前替玩家完成或选择结果。` : "这是非最终幕：继续推进中心冲突，不提前完成或宣告结局。"}`
