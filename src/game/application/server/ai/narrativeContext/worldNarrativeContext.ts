...
function outputContract(need: EvolutionNeed, storyState: StoryState): string {
  ...
  const consequenceBindingFields = `- consequenceBindings=可选绑定数组；bind_investigation 必须同时提出独立 scene 的 newLocation、同批 newFact 和 factRef="@new.fact"，discoveryMode 必须为 investigation，approaches 必须与 newFact 的 2-3 条方法逐项对应。若需要比较无额外见证与现场见证，至少一条 witnessNpcIds=[]，至少一条 witnessNpcIds=["@new.npc"]；@new.npc 只能引用同批实际创建且仍保留的新 NPC。仅写 investigationApproaches 或 automatic 事实不能开启调查。`;
