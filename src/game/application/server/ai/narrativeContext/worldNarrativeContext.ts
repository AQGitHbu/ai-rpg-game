...
function outputContract(need: EvolutionNeed): string {
  ...
  const outer = "只输出 JSON，不能解释或 Markdown。外层必须是 {\"proposal\":{...}}；proposal 必须有 beatSummary（非空且≤200字）。所有未使用字段必须完全省略，绝不写 null。NPC 的 relationshipSeeds 是可选数组，例如 {\"relationshipSeeds\":[{\"targetNpcId\":\"既有 active NPC ID\",\"stance\":\"ally\",\"reason\":\"...\"}]}；每项只能包含 targetNpcId、stance、reason，其中 targetNpcId 只能引用实体规则闭包中的既有 active NPC，stance 只能是 ally|protective_of|indebted_to|rival|wary，reason 必须非空且≤200字；不得提交 affinity、stage、evidence 或 actionId。";
