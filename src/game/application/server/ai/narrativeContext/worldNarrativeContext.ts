...
function outputContract(need: EvolutionNeed): string {
  const commonFields = `可用增量字段（只在需要时出现）：\n- newLocation={"name":"2-40字名称","description":"非空且≤200字","scale":"scene或town","placement":"world或town_building","connectFromLocationId":"已有地点ID"}\n- newNpc={"name":"2-40字名称","role":"非空且≤200字","description":"非空且≤200字","locationRef":{"kind":"existing","id":"已有地点ID"}或{"kind":"new_location"},"goals":["可选目标"]}\n- newItem={"name":"2-40字名称","description":"非空且≤200字","locationRef":"current或new_location"}\n- newEnemy={"name":"2-40字名称","tier":"normal或boss","locationRef":"current或new_location"}\n- newFact={"text":"非空且≤200字","visibility":"public或npc_private","investigationLabel":"可选，2-40字","investigationApproaches":[{"approachId":"非空ID","label":"非空提示","hint":"可选非空提示","evidenceQuality":"clean或noisy","tensionDelta":-5..20}]}；investigationApproaches 若出现，必须且只能有 2-3 条，tensionDelta=-5..20。`;
  const outer = "只输出 JSON，不能解释或 Markdown。外层必须是 {\"proposal\":{...}}；proposal 必须有 beatSummary（非空且≤200字）。所有未使用字段必须完全省略，绝不写 null。";
