...
- 若故事开端要求玩家先选择可主动调查的方法，必须实际创建一个未发现事实：给对应 publicFacts 提供非空 investigationLabel 和恰好 2–3 个 investigationApproaches，并在 opening.consequenceBindings 使用同一个 fact key 提供 bind_investigation、discoveryMode="investigation" 和同一组 approaches；不要把该事实 key 放入 opening.npc.knownFactKeys。仅写 investigationApproaches 或使用 automatic 事实不满足。若故事开端还要求区分无额外见证与现场见证，绑定 methods 中至少一条 witnessNpcIds=[]，至少一条 witnessNpcIds=["@new.npc"]；world.publicFacts 中的 approaches 只放展示字段。
