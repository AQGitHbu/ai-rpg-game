...
describe("narrativeMemoryPolicy", () => {
  it("uses the fixed P2 policy and accepts a positive prompt budget override", () => {
    expect(DEFAULT_NARRATIVE_MEMORY_POLICY).toMatchObject({ threshold: 50, batchSize: 10, rawSoftEstimatedTokens: 24_000, summarySourceMaxEstimatedTokens: 24_000, overviewMaxEstimatedTokens: 6_000, promptMaxEstimatedTokens: 64_000 });
