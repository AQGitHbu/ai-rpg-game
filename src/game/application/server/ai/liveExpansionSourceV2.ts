...
export function createLiveExpansionSourceV2(deps: ExpansionLiveDeps): ExpansionSource {
  ...
  return {
    async propose(ctx) {
      ...
      try {
        ...
        if (!result.ok) {
          logger?.warn("v2_expansion_ai_failed", { code: result.code });
          return { proposals: [] };
        }
        ...
        if (parsedProposals.length === 0) {
          logger?.warn("v2_expansion_invalid_data");
          return { proposals: [] };
        }
        const filtered = filterProposalsByExistingLocations(parsedProposals, ctx.worldState);
        return { proposals: filtered };
      } catch (error) {
        logger?.warn("v2_expansion_transport_failed", { message: (error as Error)?.message });
        return { proposals: [] };
      }
    },
  };
