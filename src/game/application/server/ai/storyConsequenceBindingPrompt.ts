...
export function storyConsequenceBindingPrompt(mode: "opening" | "decision"): string {
  ...
  return [
    ...
    mode === "opening"
      ...
      : "决策引用：使用上下文已有正式 ID；@current.location 指当前地点，@current.focus_npc 仅在存在当前焦点时可用。@new.location/@new.npc/@new.item/@new.enemy/@new.fact/@new.quest 仅指本包 worldDelta 实际创建且保留的对应实体；引用字段必须匹配实体种类。新 NPC 的 goalOrdinal 对应本包 worldDelta.newNpc.goals；已有 NPC 仅引用实体卡中提供的 goalBindings 元数据，不能猜测未提供目标。",
    ...(mode === "opening" ? [] : [
      "bind_investigation：精确为 {kind,factRef,discoveryMode,approaches}，discoveryMode 固定 investigation，事实必须未发现且位于独立 scale=scene 地点，town 不可调查。可首次绑定上下文内已有的合法未发现 scene fact，使用其正式 factId，无需新增 worldDelta；若本包新建调查现场，则由 newLocation/newFact 提供独立 scene 与事实，factRef=@new.fact。approaches 恰好 2–3 条，approachId 非空且唯一，label 非空，可选非空 hint；label/hint 不泄漏待查事实正文；evidenceQuality=clean|noisy，tensionDelta 为 [-5,20] 有限数值，可选 requirements（0–4 条）及 witnessNpcIds（去重 NPC 引用数组）。必须用 approaches 字段，不是 methods。基础字段与 newFact.investigationApproaches 的同一方法对应；requirements/witnessNpcIds 权威放在绑定 approaches。",
