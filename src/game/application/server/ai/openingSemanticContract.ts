...
export const OPENING_SEMANTIC_CONTRACT = [
  ...
  "递送契约的中心结果必须能由现有 give_item 实际完成：将唯一委托物交给最终幕绑定接应人。delivery.verificationFactKeys 指向具体的交付约定及知情来源；公开约定的收信职责、地点和普通对话确认即可，不要求身份谜题、秘密凭据、核验互动或额外核验状态。仅有‘玩家持有信筒’或‘曾接受委托’仍不足以说明应交给谁。作者可在初始事实/历史确立与设定一致的公开约定，并让需要说明约定的开局 NPC 显式具备相应知识；未来接应人仍待具象化，不提前创建实体或预写送达。",
