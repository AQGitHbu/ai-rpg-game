...
import { runtimeNarrativeResponseFormat } from "./runtimeNarrativeResponseFormat";
...
export function createRuntimeNarrativeSources(env: Record<string, string | undefined>): Readonly<{ directorSource: DirectorSource; sceneScriptSource: SceneScriptSource; npcLineSource: NpcLineSource }> {
  ...
  if (runtime.status === "available") return createLiveRuntimeNarrativeSources({ transport: createOpenAiCompatibleTransport(), config: runtime.config, responseFormat: (role) => runtimeNarrativeResponseFormat(runtime.outputFormat, role) });
