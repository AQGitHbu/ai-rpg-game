...
import { repairWorldGenerationCandidate, createWorldGenerationSourceV2 } from "./worldGenerationSourceV2";
...
describe("createWorldGenerationSourceV2", () => {
  it("无 transport 时确定性 fallback 通过同一 validator/compiler", async () => {
    const source = createWorldGenerationSourceV2({});
  ...
  it("AI 返回有效结构化候选时经机械修复 + 校验通过", async () => {
    ...
    const source = createWorldGenerationSourceV2({ transport, config: { baseUrl: "x", apiKey: "k", model: "m" } });
    const candidate = await source.generate({ gameType: "wuxia", seed: "s", gameLength: "short" });
