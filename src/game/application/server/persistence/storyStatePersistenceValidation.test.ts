...
describe("parsePersistableStoryState", () => {
  it("accepts v9 only and reconstructs memory from the supplied ledger", () => {
  ...
  it("classifies v1-v8 as unsupported and future versions as version mismatch", () => {
    const value = storyState();
    expect(parsePersistableStoryState({ ...value, version: 7 }, [])).toMatchObject({ ok: false, code: "UNSUPPORTED_RECORD" });
    // v8 缺少 branchDecisions/selectedBranches，按仓库惯例不迁移、直接归类为不支持。
    expect(parsePersistableStoryState({ ...value, version: 8 }, [])).toMatchObject({ ok: false, code: "UNSUPPORTED_RECORD" });
    expect(parsePersistableStoryState({ ...value, version: 10 }, [])).toMatchObject({ ok: false, code: "VERSION_MISMATCH" });
  });
